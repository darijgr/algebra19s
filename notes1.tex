...
%TCIDATA{LastRevised=Wednesday, February 06, 2019 09:57:48}
...
\textbf{(a)} For each $m\in\mathbb{Z}$, we have the logical equivalence%
\[
\left(  m\mid b_{i}\text{ for all }i\right)  \Longleftrightarrow\left(
m\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  \right)  .
\]


\textbf{(b)} The common divisors of $b_{1},b_{2},\ldots,b_{k}$ are exactly the
divisors of $\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $.
...
Base case ($k=0$) trivial. (Also, $k=1$ easy.)

Induction step. We induct from $k-1$ to $k$. In other words, we assume (as
induction hypothesis) that the theorem is already proven \textquotedblleft for
$k-1$ instead of $k$\textquotedblright.

Now, let's prove it for $k$.

\textbf{(d)} Let $g=\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $ and
$h=\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)
$. It suffices to prove $g\leq h$ and $h\leq g$.

\textit{Proof of }$g\leq h$\textit{:} It suffices to prove $g\mid h$, since
$g$ and $h$ are nonnegative (the case of $0$ is ruled out by easy arguments).

It suffices to prove that $g\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
$ and $g\mid b_{k}$ (because Theorem 2.41 \textbf{(a)} will then yield
$g\mid\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
,b_{k}\right)  =h$).

Well, $g$ is a common divisor of $b_{1},b_{2},\ldots,b_{k}$, thus divides
$b_{k}$. So $g\mid b_{k}$ is proven.

Also, $g\mid b_{i}$ for all $i\in\left\{  1,2,\ldots,k-1\right\}  $. Thus, by
part \textbf{(a)} applied to $k-1$ instead of $k$ (our induction hypothesis),
we get $g\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  $.

So we are done proving $g\leq h$.

\textit{Proof of }$h\leq g$\textit{:} We have%
\begin{align*}
h  & =\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
,b_{k}\right)  \\
& \mid\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  \mid b_{i}\text{ for all
}i\in\left\{  1,2,\ldots,k-1\right\}
\end{align*}
and $h\mid b_{k}$. So $h$ is a common divisor of $b_{1},b_{2},\ldots,b_{k}$.
Thus, $h$ is $\leq$ than the gcd. In other words, $h\leq g$.

Combining $h\leq g$ with $g\leq h$, we see that we are done with part
\textbf{(d)}.

\textbf{(a)} Let $m\in\mathbb{Z}$. Then, we have the equivalence%
\begin{align*}
& \ \left(  m\mid b_{i}\text{ for all }i\right)  \\
...
,\ldots,b_{k-1}\right)  \right)  \\\text{(by the induction hypothesis)}%
}}\text{ and }m\mid b_{k}\right)  \\
& \Longleftrightarrow\ \left(  m\mid\gcd\left(  b_{1},b_{2},\ldots
,b_{k-1}\right)  \text{ and }m\mid b_{k}\right)  \\
& \Longleftrightarrow\ \left(  m\mid\gcd\left(  \gcd\left(  b_{1},b_{2}%
,\ldots,b_{k-1}\right)  ,b_{k}\right)  \right)  \ \ \ \ \ \ \ \ \ \ \left(
\text{by Theorem 2.41 \textbf{(a)}}\right)  \\
& \Longleftrightarrow\ \left(  m\mid\gcd\left(  b_{1},b_{2},\ldots
,b_{k}\right)  \right)  \ \ \ \ \ \ \ \ \ \ \left(  \text{by part
\textbf{(d)}, which we just proved}\right)  .
\end{align*}


\textbf{(b)} is a restatement of \textbf{(a)}.

\textbf{(c)} is a restatement of \textbf{(b)}.

\textbf{(e)} By the induction hypothesis, we can apply \textbf{(e)} to $k-1$
instead of $k$. We see that there exist $k-1$ integers $y_{1},y_{2}%
,\ldots,y_{k-1}$ such that%
\[
\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  =y_{1}b_{1}+y_{2}b_{2}%
+\cdots+y_{k-1}b_{k-1}.
\]
Furthermore, Bezout's theorem yields that there exist two integers $u$ and $v$
such that%
\[
\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)
=u\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  +vb_{k}.
\]


Now, part \textbf{(d)} yields%
\begin{align*}
\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)    & =\gcd\left(  \gcd\left(
b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  \\
& =u\underbrace{\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  }_{=y_{1}%
b_{1}+y_{2}b_{2}+\cdots+y_{k-1}b_{k-1}}+vb_{k}\\
& =u\left(  y_{1}b_{1}+y_{2}b_{2}+\cdots+y_{k-1}b_{k-1}\right)  +vb_{k}\\
& =uy_{1}b_{1}+uy_{2}b_{2}+\cdots+uy_{k-1}b_{k-1}+vb_{k}.
\end{align*}
This is a linear combination of the $b_{1},b_{2},\ldots,b_{k}$. So part
\textbf{(e)} is proven.

We are thus done with the induction step.
\end{proof}

\subsection{Coprime integers}

\begin{definition}
Let $a,b\in\mathbb{Z}$. We say that $a$ and $b$ are \textbf{coprime} (or
\textbf{relatively prime}) if $\gcd\left(  a,b\right)  =1$. Note that this is
a symmetric relation, since $\gcd\left(  a,b\right)  =\gcd\left(  b,a\right)
$.

We write \textquotedblleft$a\perp b$\textquotedblright\ for \textquotedblleft%
$a$ is coprime to $b$\textquotedblright. (LaTeX:
...
\begin{example}
\textbf{(a)} The integers $3$ and $5$ are coprime, since%
\begin{align*}
\gcd\left(  3,5\right)    & =\gcd\left(  3,2\right)  =\gcd\left(  2,3\right)
\\
& =\gcd\left(  2,1\right)  =\gcd\left(  1,2\right)  =\gcd\left(  1,0\right)
=\left\vert 1\right\vert =1.
\end{align*}


\textbf{(b)} Each integer $a$ is coprime to $a+1$ (that is, $a$ and $a+1$ are
coprime). This is because%
\begin{align*}
\gcd\left(  a,a+1\right)    & =\gcd\left(  a,1\right)
\ \ \ \ \ \ \ \ \ \ \left(  \text{since }a+1\equiv1\operatorname{mod}a\right)
\\
& =\gcd\left(  1,a\right)  =\gcd\left(  1,0\right)
\ \ \ \ \ \ \ \ \ \ \left(  \text{since }0\equiv a\operatorname{mod}1\right)
\\
& =\left\vert 1\right\vert =1.
\end{align*}


\textbf{(c)} When is an integer $a$ coprime to $a+2$ ? Exactly when $a$ is
odd. Proof:%
\begin{align*}
\gcd\left(  a,a+2\right)    & =\gcd\left(  a,2\right)
\ \ \ \ \ \ \ \ \ \ \left(  \text{since }a+2\equiv2\operatorname{mod}a\right)
\\
& =%
\begin{cases}
...
\begin{theorem}
Let $a,b,c\in\mathbb{Z}$ be such that $a\mid bc$ and $a\perp b$. Then, $a\mid
c$.
\end{theorem}

\begin{theorem}
Let $a,b,c\in\mathbb{Z}$ be such that $a\mid c$ and $b\mid c$ and $a\perp b$.
Then, $ab\mid c$.
\end{theorem}
...
$ab\equiv1\operatorname{mod}n$. (Such a $b$ is called a \textbf{modular
inverse} of $a$ modulo $n$.)
\end{theorem}

\begin{proof}
\textbf{(a)} Bezout's theorem yields $\gcd\left(  a,n\right)  =xa+yn$ for some
$x,y\in\mathbb{Z}$. Now, $ax\equiv\gcd\left(  a,n\right)  \operatorname{mod}%
n$, since $\underbrace{\gcd\left(  a,n\right)  }_{=xa+yn}-ax=yn$ is divisible
by $n$. So take $b=x$.

\textbf{(b)} Follows from \textbf{(a)}, since $\gcd\left(  a,n\right)  =1$.
\end{proof}

\subsubsection{An application}

\begin{proposition}
Let $n\in\mathbb{N}$. Then, $1+2+\cdots+n=\dfrac{n\left(  n+1\right)  }{2}$.
\end{proposition}

\begin{proof}
We have%
\begin{align*}
& 2\cdot\left(  1+2+\cdots+n\right)  \\
& =\left(  1+2+\cdots+n\right)  +\left(  1+2+\cdots+n\right)  \\
& =\left(  1+2+\cdots+n\right)  +\left(  n+\left(  n-1\right)  +\cdots
+1\right)  \\
& =\underbrace{\left(  1+n\right)  +\left(  2+\left(  n-1\right)  \right)
+\left(  3+\left(  n-2\right)  \right)  +\cdots+\left(  n+1\right)
}_{\substack{\text{a sum of }n\text{ addends,}\\\text{each of which equals
}n+1}}\\
& =n\left(  n+1\right)  .
\end{align*}
...
Similar formulas exist for summing higher powers, but less easy to prove.

$\sum_{k=1}^{n}k=\allowbreak\dfrac{1}{2}n\left(  n+1\right)  $.

$\sum_{k=1}^{n}k^{2}=\allowbreak\dfrac{1}{6}n\left(  2n+1\right)  \left(
n+1\right)  $.

$\sum_{k=1}^{n}k^{3}=\allowbreak\dfrac{1}{4}n^{2}\left(  n+1\right)  ^{2}$.

$\sum_{k=1}^{n}k^{4}=\allowbreak\dfrac{1}{30}n\left(  2n+1\right)  \left(
n+1\right)  \left(  3n+3n^{2}-1\right)  $.

$\sum_{k=1}^{n}k^{5}=\allowbreak\dfrac{1}{12}n^{2}\left(  n+1\right)
^{2}\left(  2n+2n^{2}-1\right)  $.

In general: Fixing a $d\in\mathbb{N}$, the sum $\sum_{k=1}^{n}k^{d}$ is a
polynomial in $n$ of degree $d+1$ with rational coefficients.

\begin{exercise}
Let $n\in\mathbb{N}$ and let $d$ be an \textbf{odd} positive integer. Prove
that%
\[
...
\begin{proof}
Recall that $1+2+\cdots+n=\dfrac{n\left(  n+1\right)  }{2}$, so we need to
prove%
\[
...
\end{align*}
(because $n\perp n+1$, and we had a theorem saying that if $a,b,c\in
\mathbb{Z}$ are such that $a\mid c$ and $b\mid c$ and $a\perp b$, then $ab\mid
c$).

\textit{Proof of }$n+1\mid2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
$\textit{:}

We have%
\begin{align*}
& 2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  \\
& =\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  +\left(  1^{d}+2^{d}+\cdots
+n^{d}\right)  \\
& =\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  +\left(  n^{d}+\left(  n-1\right)
^{d}+\cdots+1^{d}\right)  \\
& =\sum_{k=1}^{n}k^{d}+\sum_{k=1}^{n}\left(  n+1-k\right)  ^{d}\\
& =\sum_{k=1}^{n}\underbrace{\left(  k^{d}+\left(  n+1-k\right)  ^{d}\right)
}_{\substack{\text{This is divisible by }k+\left(  n+1-k\right)
\text{,}\\\text{because }x^{d}+y^{d}\text{ is always divisible by
}x+y\\\text{when }d\text{ is odd(*)}}}
\end{align*}


(*) This is because we know that $x^{d}-y^{d}$ is always divisible by $x-y$
(since $x^{d}-y^{d}=\left(  x-y\right)  \left(  x^{d-1}+x^{d-2}y+x^{d-3}%
y^{2}+\cdots+y^{d-1}\right)  $). If $d$ is odd, we can plug $-y$ for $y$ here.

So we have a sum of addends that are each divisible by $n+1$. So it is
divisible by $n+1$.

\textit{Proof of }$n\mid2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  $\textit{:}

We have shown that $n+1\mid2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  $.

Apply this to $n-1$ instead of $n$. We get%
\begin{align*}
n  & \mid2\left(  1^{d}+2^{d}+\cdots+\left(  n-1\right)  ^{d}\right)
,\ \ \ \ \ \ \ \ \ \ \text{so}\\
2\left(  1^{d}+2^{d}+\cdots+\left(  n-1\right)  ^{d}\right)    &
\equiv0\operatorname{mod}n.
\end{align*}
We want%
\begin{align*}
\mathit{\ }n  & \mid2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
,\ \ \ \ \ \ \ \ \ \ \text{that is,}\\
2\left(  1^{d}+2^{d}+\cdots+n^{d}\right)    & \equiv0\operatorname{mod}n.
\end{align*}
How do we get it?
\end{proof}

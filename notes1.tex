...
%TCIDATA{LastRevised=Friday, February 15, 2019 09:55:33}
...
about lcms and gcds. For example, take our theorem that
\[
...
\begin{exercise}
You have a corridor with $1000$ lightswitches, initially all off.

Every night, a ghost glides along the corridor (always in the same direction).

During the $1$st night, the ghost toggles each switch.

During the $2$nd night, the ghost toggles switches $2,4,6,8,10,\ldots$.

During the $3$rd night, the ghost toggles switches $3,6,9,12,15,\ldots$.

And so on.

Which lights will be on after $1000$ nights?
\end{exercise}
...
[\textit{Proof of claim:} Recall the bijection%
\begin{align*}
\left\{  \text{positive divisors of }n\right\}   &  \rightarrow\left\{
\text{positive divisors of }n\right\}  ,\\
d &  \mapsto n/d.
\end{align*}
This matches each positive divisor of $n$ with another, unless the two are
actually equal. But the latter only happens if there is a positive divisor $d$
such that $n/d=d$, which means $n=d^{2}$. When $n$ is not a perfect square,
this never happens. When $n$ is a perfect square, this happens exactly once.]

So the lights $1^{2},2^{2},\ldots,31^{2}$ (and no others) will be on after the
$1000$ nights.
\end{proof}
...
\textbf{(b)} In general, $a^{p}\equiv a\operatorname{mod}p$.
\end{theorem}

\begin{remark}
Part \textbf{(b)} follows from \textbf{(a)}, because:

\begin{itemize}
\item if $p\nmid a$, then \textbf{(a)} yields $a^{p-1}\equiv
1\operatorname{mod}p$, thus $a^{p}=aa^{p-1}\equiv a1=a\operatorname{mod}p$.

\item if $p\mid a$, then both $a^{p}$ and $a$ are $\equiv0\operatorname{mod}%
p$, so $a^{p}\equiv a\operatorname{mod}p$.
\end{itemize}

Conversely, part \textbf{(a)} follows from \textbf{(b)}, because: Assume
$p\nmid a$. Then, $p\perp a$, so $a\perp p$. Thus, $a$ has a modular inverse
$a^{\prime}$ modulo $p$. So $aa^{\prime}\equiv1\operatorname{mod}p$. Thus, we
can \textquotedblleft cancel\textquotedblright\ $a$ from any congruence modulo
$p$ by multiplying the congruence with $a^{\prime}$. Doing this with
$a^{p}\equiv a\operatorname{mod}p$ (which follows from \textbf{(b)}), we
obtain $a^{p-1}\equiv1\operatorname{mod}p$.
\end{remark}

\begin{theorem}
(Euler's Theorem)

Let $n$ be a positive integer. Let $a\in\mathbb{Z}$ be coprime to $n$.

Then, $a^{\phi\left(  n\right)  }\equiv1\operatorname{mod}n$.
\end{theorem}

Euler's theorem yields part \textbf{(a)} of FLT, since $\phi\left(  p\right)
=p-1$ when $p$ is prime.
...
Notational remark: $a^{b^{c}}$ means $a^{\left(  b^{c}\right)  }$, not
$\left(  a^{b}\right)  ^{c}=a^{bc}$.
\end{exercise}

\begin{proof}
[Solution.] The last digit (of a positive integer) is the remainder modulo
$10$. So we compute modulo $10$.

Euler's theorem yields $3^{\phi\left(  10\right)  }\equiv1\operatorname{mod}%
10$ since $3\perp10$.

In view of $\phi\left(  10\right)  =4$, this becomes $3^{4}\equiv
1\operatorname{mod}10$.

Now, $3^{4^{5}}=3^{4\cdot4^{4}}=\left(  \underbrace{3^{4}}_{\equiv
...
\begin{proof}
\textit{First proof:} If $n=1$, then this is easily done by hand. Otherwise,
neither $0$ nor $n$ is coprime to $n$, so the sets
\begin{align*}
...
\textit{Second proof:} $0$ is coprime to $n$ if and only if $n$ is coprime to
$n$. So removing $n$ from the set $\left\{  1,2,\ldots,n\right\}  $ and adding
$0$ to it does not change the number of elements coprime to $n$.
\end{proof}

\begin{proof}
[Proof of Euler's theorem.] Lemma says that%
\[
\phi\left(  n\right)  =\left\vert \left\{  i\in\left\{  0,1,\ldots
,n-1\right\}  \ \mid\ i\perp n\right\}  \right\vert .
\]
So let $C:=\left\{  i\in\left\{  0,1,\ldots,n-1\right\}  \ \mid\ i\perp
n\right\}  $.
...
[\textit{Proof:} Let $i\in C$. Then, $i\perp n$ and thus $ai\perp n$ (since
$a\perp n$) and thus $\gcd\left(  ai,n\right)  =1$, but therefore $\gcd\left(
n,\left(  ai\right)  \%n\right)  =\gcd\left(  n,ai\right)  =\gcd\left(
ai,n\right)  =1$, and thus $\left(  ai\right)  \%n\perp n$. Therefore,
$\left(  ai\right)  \%n\in C$ because $\left(  ai\right)  \%n\in\left\{
0,1,\ldots,n-1\right\}  $.]

Furthermore, the map $f$ is injective.

[\textit{Proof:} Let $i\in C$ and $j\in C$ be such that $f\left(  i\right)
=f\left(  j\right)  $. We must prove that $i=j$.

We can rewrite $f\left(  i\right)  =f\left(  j\right)  $ as $\left(
ai\right)  \%n=\left(  aj\right)  \%n$. By an old exercise (HW1), this is
saying that $ai\equiv aj\operatorname{mod}n$. But $a$ has a modular inverse
modulo $n$ (since $a\perp n$), so we can cancel $a$ from this congruence and
obtain $i\equiv j\operatorname{mod}n$. Since both $i$ and $j$ lie in $\left\{
0,1,\ldots,n-1\right\}  $, so this entails $i=j$. Qed.]

Now we could prove that $f$ is surjective as well. But we can also get it for
free: The map $f$ is an injective map between two finite sets of the same size
(namely, $C$ and $C$). This automatically implies that $f$ is bijective, by
the so-called \textit{Pigeonhole Principle for Injections}:

\begin{statement}
\textit{Pigeonhole Principle for Injections:} Let $A$ and $B$ be two finite
sets such that $\left\vert A\right\vert \geq\left\vert B\right\vert $. Then,
any injective map from $A$ to $B$ is bijective.
\end{statement}

\begin{statement}
\textit{Pigeonhole Principle for Surjections:} Let $A$ and $B$ be two finite
sets such that $\left\vert A\right\vert \leq\left\vert B\right\vert $. Then,
any surjective map from $A$ to $B$ is bijective.
\end{statement}

Now, we know that $f$ is bijective. Consider the product $\prod\limits_{c\in
C}c$. We can substitute $f\left(  c\right)  $ for $c$ in it, since $f$ is
bijective. Thus,%
\begin{align*}
\prod\limits_{c\in C}c  & =\prod\limits_{c\in C}\underbrace{f\left(  c\right)
...
\prod_{c\in C}c\operatorname{mod}n
\end{align*}
(since $\left\vert C\right\vert =\phi\left(  n\right)  $). Now, each $c\in C$
is coprime to $n$ (by definition of $C$). Hence, $\prod_{c\in C}c$ is coprime
to $n$ (by Exercise \ref{exe.ent.coprime.ab-to-ck}). Hence, we can cancel this
product from the congruence above. We get $1\equiv a^{\phi\left(  n\right)
}\operatorname{mod}n$. This is Euler's theorem.

Thus, FLT follows.
\end{proof}

\begin{remark}
Which fractions $\dfrac{a}{b}$ of integers have a \textbf{purely periodic}
decimal expansion? \textquotedblleft Purely periodic\textquotedblright\ means
\textquotedblleft the repeating part starts right after the decimal
point\textquotedblright.

$\dfrac{1}{6}=\allowbreak0.16666666\ldots$ is not purely periodic.

$\dfrac{1}{7}=\allowbreak0.142857142857142857\ldots$ is purely periodic.

$\dfrac{1}{5}=\allowbreak0.2000000\ldots$ is not purely periodic.

The fraction $\dfrac{a}{b}$ (with $a\perp b$) is purely periodic if and only
if $b\perp10$ (which is equivalent to $2\nmid b$ and $5\nmid b$). This can be
proven using Euler's theorem: see Keith Conrad, \textit{Euler's theorem}.
\end{remark}
...
If $p$ is a prime, then $\left(  p-1\right)  !\equiv-1\operatorname{mod}p$.
\end{theorem}

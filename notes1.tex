...
%TCIDATA{LastRevised=Monday, February 04, 2019 09:58:37}
...
\begin{definition}
Let $b\in\mathbb{Z}$. The \textbf{divisors} of $b$ are defined as the integers
that divide $b$.
\end{definition}
...
\textbf{(b)} All the integers are divisors of $0$.
...
\begin{definition}
Let $b_{1},b_{2},\ldots,b_{k}$ be finitely many integers.

The \textbf{common divisors }of $b_{1},b_{2},\ldots,b_{k}$ are the integers
that divide all of $b_{1},b_{2},\ldots,b_{k}$. We let $\operatorname*{Div}%
\left(  b_{1},b_{2},\ldots,b_{k}\right)  $ be the set of these common divisors.
\end{definition}

\begin{example}%
\begin{align*}
\operatorname*{Div}\left(  6,14\right)    & =\underbrace{\operatorname*{Div}%
\left(  6\right)  }_{=\left\{  1,2,3,6,-1,-2,-3,-6\right\}  }\cap
\underbrace{\operatorname*{Div}\left(  14\right)  }_{=\left\{
1,2,7,14,-1,-2,-7,-4\right\}  }\\
& =\left\{  1,2,-1,-2\right\}  .
\end{align*}
%

\[
\operatorname*{Div}\left(  6,15,10\right)  =\left\{  1,-1\right\}  .
\]
Note that $6$ and $15$ have common divisor $3$;

$6$ and $10$ have common divisor $2$;

$15$ and $10$ have common divisor $5$.
\end{example}

Note that if $b\in\mathbb{Z}$, then $\operatorname*{Div}\left(  b\right)
=\left\{  \text{divisors of }b\right\}  $.

\begin{proposition}
Let $b_{1},b_{2},\ldots,b_{k}$ be $k$ integers that are not all $0$. Then, the
set $\operatorname*{Div}\left(  b_{1},b_{2},\ldots,b_{k}\right)  $ is a
nonempty finite set and has a largest element; this largest element is positive.
\end{proposition}

\begin{definition}
Let $b_{1},b_{2},\ldots,b_{k}$ be $k$ integers that are not all $0$. Then, the
\textbf{greatest common divisor} of $b_{1},b_{2},\ldots,b_{k}$ is the largest
element of $\operatorname*{Div}\left(  b_{1},b_{2},\ldots,b_{k}\right)  $. We
call it $\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $, and also use
\textquotedblleft\textbf{gcd}\textquotedblright\ as shorthand for
\textquotedblleft greatest common divisor\textquotedblright.

If $b_{1},b_{2},\ldots,b_{k}$ are all $0$, then we set $\gcd\left(
b_{1},b_{2},\ldots,b_{k}\right)  =0$.
\end{definition}

\begin{remark}
We will later see that, e.g., any five integers $a,b,c,d,e$ satisfy%
\[
\gcd\left(  a,b,c,d,e\right)  =\gcd\left(  a,\gcd\left(  b,\gcd\left(
c,\gcd\left(  d,e\right)  \right)  \right)  \right)  .
\]
So gcd's of two numbers are the most important case.
\end{remark}
...
(But it is not true that $\gcd\left(  vb+a,ua+b\right)  =\gcd\left(
a,b\right)  $ for all $a,b,u\in\mathbb{Z}$. Exercise?)

\textbf{(d)} If $a,b,c\in\mathbb{Z}$ satisfy $b\equiv c\operatorname{mod}a$,
then $\gcd\left(  a,b\right)  =\gcd\left(  a,c\right)  $.

\textbf{(e)} We have $\gcd\left(  a,b\right)  \mid a$ and $\gcd\left(
a,b\right)  \mid b$ for all $a,b\in\mathbb{Z}$.
\end{proposition}

\begin{proof}
\textbf{(c)} Let $a,b,u\in\mathbb{Z}$. WLOG assume that $a,b$ are not both $0$.

Claim: $\operatorname*{Div}\left(  a,ua+b\right)  =\operatorname*{Div}\left(
a,b\right)  $.

Indeed, let $d\in\operatorname*{Div}\left(  a,ua+b\right)  $. We shall prove
that $d\in\operatorname*{Div}\left(  a,b\right)  $.

We have $d\in\operatorname*{Div}\left(  a,ua+b\right)  $, so that $d\mid a$
and $d\mid ua+b$. We must prove that $d\mid b$, since this will yield
$d\in\operatorname*{Div}\left(  a,b\right)  $.

We have%
\[
b=\underbrace{\left(  ua+b\right)  }_{\equiv0\operatorname{mod}d}%
-u\underbrace{a}_{\equiv0\operatorname{mod}d}\equiv0-u0=0\operatorname{mod}d.
\]
(To be rigorous: Multiplying $u\equiv u\operatorname{mod}d$ with
$a\equiv0\operatorname{mod}d$, we get $ua\equiv u0=0\operatorname{mod}d$.
Subtracting this congruence from $ua+b\equiv0\operatorname{mod}d$, and get
$\left(  ua+b\right)  -ua\equiv0-0=0\operatorname{mod}d$.)

So $d\mid b$, so $d\in\operatorname*{Div}\left(  a,b\right)  $.

Thus, we have proven $\operatorname*{Div}\left(  a,ua+b\right)  \subseteq
\operatorname*{Div}\left(  a,b\right)  $.

Similarly, $\operatorname*{Div}\left(  a,b\right)  \subseteq
\operatorname*{Div}\left(  a,ua+b\right)  $.

Combining these, we get $\operatorname*{Div}\left(  a,ua+b\right)
=\operatorname*{Div}\left(  a,b\right)  $.

Hence, $\gcd\left(  a,ua+b\right)  =\gcd\left(  a,b\right)  $. So part
\textbf{(c)} is proven.

\textbf{(d)} Let $a,b,c\in\mathbb{Z}$ satisfy $b\equiv c\operatorname{mod}a$.
Then, $a\mid b-c$, so that $b-c=av$ for some $v\in\mathbb{Z}$. Thus,
$c=b-av=\left(  -v\right)  a+b$. So
\[
\gcd\left(  a,c\right)  =\gcd\left(  a,\left(  -v\right)  a+b\right)
=\gcd\left(  a,b\right)
\]
(by part \textbf{(c)}). So \textbf{(d)} is proven.
\end{proof}

The above proposition was easy, but highly useful for computing $\gcd\left(
a,b\right)  $. Example:%
\begin{align*}
& \gcd\left(  210,45\right)  \overset{\mathbf{(b)}}{=}\gcd\left(
45,210\right)  \overset{\mathbf{(d)}}{=}\gcd\left(  45,30\right)  \\
& \ \ \ \ \ \ \ \ \ \ \left(  \text{since }210\equiv30\operatorname{mod}%
45\right)  \\
& \overset{\mathbf{(b)}}{=}\gcd\left(  30,45\right)  \overset{\mathbf{(d)}%
}{=}\gcd\left(  30,15\right)  \overset{\mathbf{(b)}}{=}\gcd\left(
15,30\right)  \\
& \overset{\mathbf{(d)}}{=}\gcd\left(  15,0\right)  \overset{\mathbf{(a)}%
}{=}\left\vert 15\right\vert =15.
\end{align*}
This is called the \textit{Euclidean algorithm}.

\begin{theorem}
Let $a$ and $b$ be two integers. Then, there exist integers $x$ and $y$ such
that $\gcd\left(  a,b\right)  =xa+yb$.
\end{theorem}

In other words, $\gcd\left(  a,b\right)  $ is a $\mathbb{Z}$-linear
combination of $a$ and $b$.

Rough idea of the proof (see the notes for details):

\begin{itemize}
\item It suffices to prove the theorem for $a,b\geq0$.

\item Strong induction on $a+b$. This means we fix $n\in\mathbb{N}$, assume
the theorem to hold for $a+b<n$, and prove that it holds for $a+b=n$.

In proving this, we can WLOG assume that $a\leq b$. Then,%
\[
\gcd\left(  a,b\right)  =\gcd\left(  a,b-a\right)
\]
(by part \textbf{(d)} of the proposition, since $b-a\equiv b\operatorname{mod}%
a$). But $a+\left(  b-a\right)  =b<a+b=n$, so the induction hypothesis tells
us that the theorem is already proven for $b-a$ instead of $a$. Then see the consequences.

...
%TCIDATA{LastRevised=Monday, February 11, 2019 09:55:16}
...
\ref{cor.ent.quo-rem.remmod} \textbf{(c)} (applied to $u=2$) says that if
...
u+1\right)  $ has \textbf{at least} one even factor. But a product of any
integer with an even integer is even\footnote{\textit{Proof.} We must prove
that if $a$ is an even integer, and if $b$ is an integer, then the product
$ab$ is even.
\par
...
solves Exercise \ref{exe.ent.even-odd.1} \textbf{(g)}.
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.gcd.sa,sb,sc}.]TODO! (HW2)
\end{proof}
...
\begin{proof}
[Proof of Theorem \ref{thm.ent.lcm.uniprop-mul}.]It is not hard to transform
our above proof of Theorem \ref{thm.ent.gcd.uniprop-mul} into a proof of
Theorem \ref{thm.ent.lcm.uniprop-mul}. To do so, we need (of course) to flip
the divisibility relations and replace \textquotedblleft
divisor\textquotedblright\ by \textquotedblleft multiple\textquotedblright%
...
a multiple of the integer $m/d$ (since $d\mid m$ and thus $m/d\in\mathbb{Z}$
and of course $m/d\mid m$), and therefore $m$ has already been removed in Step
$m/d$ (which has already happened before Step $d$ (because $d^{2}>30\geq m$
and therefore $d>m/d$)). Thus, we only need to take the Steps $d$ with
$d^{2}\leq30$.
\end{itemize}
...
1,2,\ldots,p-1\right\}  $. We must prove that $\gcd\left(  i,p\right)  =1$.

Well, $\gcd\left(  i,p\right)  $ is a positive divisor of $p$, thus must be
either $1$ or $p$ (since $p$ is prime). If it is $1$, then we are done. So
assume it is $p$ instead.

But $\gcd\left(  i,p\right)  \mid i$ and thus $\gcd\left(  i,p\right)  \leq
i\leq p-1<p$, so it cannot be $p$.
\end{proof}
...
Now, $a\%p\in\left\{  0,1,\ldots,p-1\right\}  $ and $a\%p\equiv
a\operatorname{mod}p$. Also, $a\%p\neq0$ because $p\nmid a$. Thus,
$a\%p\in\left\{  0,1,\ldots,p-1\right\}  \setminus\left\{  0\right\}
=\left\{  1,2,\ldots,p-1\right\}  $. So the previous proposition (applied to
$i=a\%p$) yields that $a\%p$ is coprime to $p$. In other words, $\gcd\left(
p,a\%p\right)  =1$. But Proposition 2.9.7 \textbf{(e)} yields $\gcd\left(
p,a\right)  =\gcd\left(  p,a\%p\right)  =1$, so that $p\perp a$. Contradiction.
\end{proof}
...
\begin{proof}
Assume the contrary. Thus, $p\nmid a$ and $p\nmid b$.

The previous proposition yields either $p\mid a$ or $p\perp a$. Thus, $p\perp
a$ (since $p\nmid a$).

But $p\mid ab$. Thus, Theorem 2.10.6 (applied to $p$, $a$ and $b$ instead of
$a$, $b$ and $c$) yields $p\mid b$. This contradicts $p\nmid b$.
\end{proof}

\begin{proposition}
Let $p$ be a prime. Let $a_{1},a_{2},\ldots,a_{k}$ be integers such that
$p\mid a_{1}a_{2}\cdots a_{k}$. Then, $p\mid a_{i}$ for some $i\in\left\{
1,2,\ldots,k\right\}  $.
\end{proposition}

\begin{proof}
Induction using the previous theorem. See the notes for details.
\end{proof}

\begin{proposition}
Let $n>1$ be an integer. Then, there exists at least one prime $p$ such that
$p\mid n$.
\end{proposition}

\begin{proof}
Let $d$ be the smallest divisor of $n$ that is $>1$. (Note that $d$ exists,
since $n$ itself is $>1$.)

Claim: $d$ is prime. Indeed, if $d$ had a divisor other than $1$ and $d$, then
this divisor would contradict the minimality of $d$.
\end{proof}

\begin{proposition}
Let $n$ be a positive integer. Then, $n$ can be written as a product of
finitely many primes.
\end{proposition}

\begin{example}
\textbf{(a)} The integer $60=\allowbreak2\cdot2\cdot3\cdot5$.

\textbf{(b)} $1=\left(  \text{empty product}\right)  $.
\end{example}

\begin{proof}
Strong induction on $n$.

We fix a positive integer $N$, and assume the proposition holds whenever
$n<N$. We must prove it for $n=N$. In other words, we must show that $N$ can
be written as a product of finitely many primes.

The proposition before says that there exists at least one prime $p$ such that
$p\mid N$. Thus, $N/p$ is a positive integer smaller than $N$. Thus, the
proposition holds for $n=N/p$ (by ind. hyp.). In other words, $N/p$ can be
written as a product of finitely many primes. Thus, the same holds for $N$
(just insert $p$ into the product). Induction complete.
\end{proof}

\begin{definition}
Let $n$ be a positive integer. Then, $n$ can be written as a product of
finitely many primes (as we just saw). Such a representation is called a
\textbf{prime factorization} of $n$.

More formally: A \textbf{prime factorization} of $n$ means a tuple $\left(
p_{1},p_{2},\ldots,p_{k}\right)  $ of primes such that $n=p_{1}p_{2}\cdots
p_{k}$.
\end{definition}

\begin{example}
The prime factorizations of $12$ are%
\[
\left(  2,2,3\right)  ,\qquad\left(  2,3,2\right)  ,\qquad\left(
3,2,2\right)  .
\]

\end{example}

We may suspect that all prime factorizations of a given number $n$ are equal
up to order (i.e., are permutations of each other). This is indeed true, and
we are going to prove this soon.

\begin{definition}
Let $p$ be a prime.

\textbf{(a)} Let $n$ be a nonzero integer. Let $v_{p}\left(  n\right)  $
(called the $p$\textbf{-valuation} or the $p$\textbf{-adic valuation} of $n$)
denotes the largest $m\in\mathbb{N}$ such that $p^{m}\mid n$. This is
well-defined, since $p^{m}\mid n$ implies $\left\vert n\right\vert \geq p^{m}$.

\textbf{(b)} Set $v_{p}\left(  0\right)  =\infty$, where $\infty$ is a symbol
which we take to satisfy%
\begin{align*}
\infty+k  & =\infty;\ \ \ \ \ \ \ \ \ \ \min\left(  S\cup\left\{
\infty\right\}  \right)  =\min S\text{ if }S\neq\varnothing;\\
\max\left(  S\cup\left\{  \infty\right\}  \right)    & =\infty
;\ \ \ \ \ \ \ \ \ \ k\leq\infty\text{ for each }k\in\mathbb{N}.
\end{align*}
(Careful: $\infty-\infty$ is not defined.)
\end{definition}

\begin{example}
$v_{5}\left(  50\right)  =2$.

$v_{5}\left(  51\right)  =0$.

$v_{5}\left(  55\right)  =1$.

$v_{5}\left(  0\right)  =\infty$.
\end{example}

\begin{lemma}
Let $p$ be a prime. Let $i\in\mathbb{N}$. Let $n\in\mathbb{Z}$. Then,
$p^{i}\mid n$ if and only if $v_{p}\left(  n\right)  \geq i$.
\end{lemma}

\begin{proof}
First, we WLOG assume that $n\neq0$ (since otherwise, $p^{i}\mid n$ and
$v_{p}\left(  n\right)  \geq i$ are both true). Now:

$\Longrightarrow:$ Assume $p^{i}\mid n$. Thus, $i$ is an $m\in\mathbb{N}$ such
that $p^{m}\mid n$. But $v_{p}\left(  n\right)  $ is the \textbf{largest} such
$m$. Hence, $v_{p}\left(  n\right)  \geq i$.

$\Longleftarrow:$ Assume $v_{p}\left(  n\right)  \geq i$. Hence, $p^{i}\mid
p^{v_{p}\left(  n\right)  }$.

But $p^{v_{p}\left(  n\right)  }\mid n$ (since $v_{p}\left(  n\right)  $ is
the largest $m\in\mathbb{N}$ such that $p^{m}\mid n$).

So $p^{i}\mid p^{v_{p}\left(  n\right)  }\mid n$.
\end{proof}

\begin{theorem}
Let $p$ be a prime, and $a$ and $b$ two integers.

\textbf{(a)} We have $v_{p}\left(  ab\right)  =v_{p}\left(  a\right)
+v_{p}\left(  b\right)  $.

\textbf{(b)} We have $v_{p}\left(  a+b\right)  \geq\min\left\{  v_{p}\left(
a\right)  ,v_{p}\left(  b\right)  \right\}  $.
\end{theorem}

\begin{proof}
\textbf{(a)} WLOG assume that $a,b$ are nonzero.

The definition of $v_{p}\left(  a\right)  $ yields that $p^{v_{p}\left(
a\right)  }\mid a$ but $p^{v_{p}\left(  a\right)  +1}\nmid a$.

Hence, $x:=a/p^{v_{p}\left(  a\right)  }$ is an integer but is not divisible
by $p$. Thus, $x\perp p$ (by our previous proposition).

Similarly, $y:=b/p^{v_{p}\left(  b\right)  }$ is an integer and $y\perp p$.

From $x\perp p$ and $y\perp p$, we obtain $xy\perp p$.

Now,
\[
ab/p^{v_{p}\left(  a\right)  +v_{p}\left(  b\right)  }=\underbrace{\left(
a/p^{v_{p}\left(  a\right)  }\right)  }_{=x}\cdot\underbrace{\left(
b/p^{v_{p}\left(  b\right)  }\right)  }_{=y}=xy
\]
is an integer coprime to $p$, hence not divisible by $p$. This means that
$p^{v_{p}\left(  a\right)  +v_{p}\left(  b\right)  }\mid ab$ but no larger
power of $p$ divides $ab$. This means $v_{p}\left(  ab\right)  =v_{p}\left(
...
\begin{corollary}
Let $p$ be a prime. Let $a_{1},a_{2},\ldots,a_{k}$ be $k$ integers. Then,
$v_{p}\left(  a_{1}a_{2}\cdots a_{k}\right)  =v_{p}\left(  a_{1}\right)
+v_{p}\left(  a_{2}\right)  +\cdots+v_{p}\left(  a_{k}\right)  $.
\end{corollary}

\begin{proof}
Induction, using the preceding theorem.
\end{proof}

\begin{proposition}
Let $n$ be a positive integer. Let $\left(  a_{1},a_{2},\ldots,a_{k}\right)  $
be a prime factorization of $n$. Let $p$ be a prime. Then, $p$ appears exactly
$v_{p}\left(  n\right)  $ many times in $\left(  a_{1},a_{2},\ldots
,a_{k}\right)  $.
\end{proposition}

\begin{proof}
We have $n=a_{1}a_{2}\cdots a_{k}$ (since $\left(  a_{1},a_{2},\ldots
,a_{k}\right)  $ is a prime factorization of $n$). Thus,
\begin{align*}
v_{p}\left(  n\right)    & =v_{p}\left(  a_{1}a_{2}\cdots a_{k}\right)  \\
& =v_{p}\left(  a_{1}\right)  +v_{p}\left(  a_{2}\right)  +\cdots+v_{p}\left(
a_{k}\right)  \ \ \ \ \ \ \ \ \ \ \left(  \text{by Corollary above}\right)
\\
& =\sum_{i=1}^{k}\underbrace{v_{p}\left(  a_{i}\right)  }_{\substack{=%
\begin{cases}
1, & \text{if }a_{i}=p;\\
0, & \text{if }a_{i}\neq p
\end{cases}
\\\text{(since }a_{i}\text{ is prime)}}}=\sum_{i=1}^{k}%
\begin{cases}
1, & \text{if }a_{i}=p;\\
0, & \text{if }a_{i}\neq p
\end{cases}
\\
& =\left(  \text{the number of }i\text{'s such that }a_{i}=p\right)  \\
...
\begin{theorem}
(Fundamental Theorem of Arithmetic) Let $n$ be a positive integer.
...
\begin{proof}
\textbf{(a)} Proven above.

\textbf{(b)} Let $\left(  a_{1},a_{2},\ldots,a_{k}\right)  $ and $\left(
b_{1},b_{2},\ldots,b_{\ell}\right)  $ be two prime factorizations of $n$.
Then, any prime $p$ appears equally often in $\left(  a_{1},a_{2},\ldots
,a_{k}\right)  $ and in $\left(  b_{1},b_{2},\ldots,b_{\ell}\right)  $
(because it appears $v_{p}\left(  n\right)  $ many times in each of these
factorizations, according to the Proposition we just proved). Thus, the
factorizations $\left(  a_{1},a_{2},\ldots,a_{k}\right)  $ and $\left(
b_{1},b_{2},\ldots,b_{\ell}\right)  $ have the same entries appear equally
often. Thus, they only differ in the order of their entries.
\end{proof}
...
Here, the product is infinite (since there are infinitely many primes -- see
below), but all but finitely many of its factors equal $1$ and thus can be
thrown away.
\end{corollary}

This expression $n=\prod_{p\text{ prime}}p^{v_{p}\left(  n\right)  }$ is
called the \textbf{canonical factorization} of $n$.

\begin{definition}
Let $n$ be a positive integer. The \textbf{prime divisors} (aka \textbf{prime
factors}) of $n$ are just the primes $p$ that divide $n$.
\end{definition}

\begin{proposition}
Let $n$ and $m$ be integers. Then, $n\mid m$ if and only if each prime $p$
satisfies $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  $.
\end{proposition}

\begin{proof}
We WLOG assume that $n$ and $m$ are nonzero.

We WLOG assume that $n$ and $m$ are positive.

Then, $n=\prod_{p\text{ prime}}p^{v_{p}\left(  n\right)  }$ and $m=\prod
_{p\text{ prime}}p^{v_{p}\left(  m\right)  }$. This yields the $\Longleftarrow
$ implication.

$\Longrightarrow:$ If $n\mid m$, then you can divide $m$ at least as many
times by $p$ as you can divide $n$. So $v_{p}\left(  n\right)  \leq
v_{p}\left(  m\right)  $.
\end{proof}

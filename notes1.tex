...
%TCIDATA{LastRevised=Wednesday, February 13, 2019 09:53:56}
...
Let $P$ be the set of all primes. Let $p\in P$. Thus, $p$ is a prime. Hence,
...
$v_{p}\left(  m\right)  <\infty=v_{p}\left(  n\right)  $, so that we don't
have $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  $.
\par
Also, $n\mid m$ does not hold (because $m$ is nonzero). Thus, the statements
\textquotedblleft$n\mid m$\textquotedblright\ and \textquotedblleft each prime
$p$ satisfies $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)
$\textquotedblright\ are both false. Hence, $n\mid m$ if and only if each
prime $p$ satisfies $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  $. In
other words, Proposition \ref{prop.ent.primes.n|m} is true. Qed.}. Hence, for
the rest of this proof, we WLOG assume that $m\neq0$. Therefore, $m$ is nonzero.

WLOG assume that $m$ and $n$ are positive (since we can replace $n$ and $m$ by
$\left\vert n\right\vert $ and $\left\vert m\right\vert $, and use Exercise
\ref{exe.ent.prime.vp-abs}).

$\Longleftarrow:$ Assume that each prime $p$ satisfies $v_{p}\left(  n\right)
\leq v_{p}\left(  m\right)  $.

But Corollary \ref{cor.ent.primes.can-fac} yields%
\[
n=\prod_{p\text{ prime}}p^{v_{p}\left(  n\right)  }%
...
+\underbrace{v_{p}\left(  b\right)  }_{\geq0}\geq v_{p}\left(  n\right)  $, so
that $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  $.
\end{proof}
...
\begin{proposition}
Let $n$ and $m$ be two nonzero integers. Then,%
\begin{align*}
\gcd\left(  n,m\right)    & =\prod_{p\text{ prime}}p^{\min\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }%
\ \ \ \ \ \ \ \ \ \ \text{and}\\
\operatorname{lcm}\left(  n,m\right)    & =\prod_{p\text{ prime}}%
p^{\max\left\{  v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }.
\end{align*}

\end{proposition}

\begin{proof}
The integer $g:=\prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(
n\right)  ,v_{p}\left(  m\right)  \right\}  }$ is a common divisor of $n$ and
$m$.

(Indeed, let us show that $g\mid n$. For that, recall $n=\prod\limits_{p\text{
prime}}p^{v_{p}\left(  n\right)  }$. So we need to show that $\prod
\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  }\mid\prod\limits_{p\text{ prime}}p^{v_{p}\left(
n\right)  }$. But this follows because each prime $p$ satisfies $\min\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  \leq v_{p}\left(
n\right)  $ and thus $p^{\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  }\mid p^{v_{p}\left(  n\right)  }$. So $g\mid n$.
Similarly, $g\mid m$. Thus, $g$ is a common divisor of $n$ and $m$.)

Hence, $g\mid\gcd\left(  n,m\right)  $.

On the other hand, let $h=\gcd\left(  n,m\right)  $. Fix any prime $p$. Then,
$h\mid n$, and thus $v_{p}\left(  h\right)  \leq v_{p}\left(  n\right)  $ (by
the Proposition above). Similarly, $v_{p}\left(  h\right)  \leq v_{p}\left(
m\right)  $. Combining these two inequalities, we obtain%
\[
v_{p}\left(  h\right)  \leq\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  .
\]
Hence, $p^{v_{p}\left(  h\right)  }\mid p^{\min\left\{  v_{p}\left(  n\right)
,v_{p}\left(  m\right)  \right\}  }$. Since we have proven this for any prime
$p$, we thus conclude that $\prod\limits_{p\text{ prime}}p^{v_{p}\left(
h\right)  }\mid\prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(
n\right)  ,v_{p}\left(  m\right)  \right\}  }$. Thus,%
\begin{align*}
h  & =\prod\limits_{p\text{ prime}}p^{v_{p}\left(  h\right)  }%
\ \ \ \ \ \ \ \ \ \ \left(  \text{by canonical factorization}\right)  \\
& \mid\prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(  n\right)
,v_{p}\left(  m\right)  \right\}  }=g.
\end{align*}
Combined with $g\mid\gcd\left(  n,m\right)  =h$, this yields $\left\vert
g\right\vert =\left\vert h\right\vert $. Thus, $g=h$ (since $g$ and $h$ are
positive). In other words, $\prod\limits_{p\text{ prime}}p^{\min\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }=\gcd\left(
n,m\right)  $. This proves the first equation.

The proof of the second equation is entirely symmetric.
\end{proof}

For example:%
\begin{align*}
& \gcd\left(  3^{2}\cdot5\cdot7^{8},2\cdot3^{3}\cdot7^{2}\right)  \\
& =2^{\min\left\{  0,1\right\}  }\cdot3^{\min\left\{  2,3\right\}  }%
\cdot5^{\min\left\{  1,0\right\}  }\cdot7^{\min\left\{  8,2\right\}  }\\
& =2^{0}\cdot3^{2}\cdot5^{0}\cdot7^{2}=3^{2}\cdot7^{2}%
\end{align*}
and%
\begin{align*}
& \operatorname{lcm}\left(  3^{2}\cdot5\cdot7^{8},2\cdot3^{3}\cdot
7^{2}\right)  \\
& =2^{\max\left\{  0,1\right\}  }\cdot3^{\max\left\{  2,3\right\}  }%
\cdot5^{\max\left\{  1,0\right\}  }\cdot7^{\max\left\{  8,2\right\}  }\\
& =2^{1}\cdot3^{3}\cdot5^{1}\cdot7^{8}.
\end{align*}


Similar results hold for $k$ numbers.

\begin{remark}
We can use the above proposition to reprove certain facts about lcms and gcds.
For example, take our theorem that
\[
\gcd\left(  a,b\right)  \cdot\operatorname{lcm}\left(  a,b\right)  =\left\vert
ab\right\vert
\]
for all $a,b\in\mathbb{Z}$. WLOG assume that $a,b\neq0$. Now, the proposition
yields%
\begin{align*}
...
=\left\vert abc\right\vert ;\\
\operatorname{lcm}\left(  a,b,c\right)  \cdot\gcd\left(  bc,ca,ab\right)    &
=\left\vert abc\right\vert ;\\
\gcd\left(  a,b,c,d\right)  \cdot\operatorname{lcm}\left(
bcd,cda,dab,abc\right)    & =\left\vert abcd\right\vert .
\end{align*}
For example, the last equation is because%
\begin{align*}
...
\begin{proposition}
Let $n$ and $m$ be two integers. Then, $n\perp m$ if and only if there exists
no prime $p$ that divides both $n$ and $m$.
\end{proposition}

\begin{proof}
$\Longrightarrow:$ Assume that $n\perp m$. Thus, $\gcd\left(  n,m\right)  =1$.
Now, if a prime $p$ would divide both $n$ and $m$, then it would also divide
$\gcd\left(  n,m\right)  =1$; but a prime $p$ cannot divide $1$. So there
exists no prime $p$ that divides both $n$ and $m$.
...
& \ \ \ \ \ \ \ \ \ \ \left(  \text{since }v_{p}\left(  n\right)  v_{p}\left(
m\right)  \geq0\text{ for all }p\right)  \\
...
We shall find a further prime distinct from each of $p_{1},p_{2},\ldots,p_{k}$.

Indeed, let $n=p_{1}p_{2}\cdots p_{k}+1$. Then, $n>1$. Hence, $n$ has a prime
divisor $p$. Consider this $p$.

We have $p\neq p_{i}$ for each $i$.

[\textit{Proof:} Assume $p=p_{i}$. Thus, $p\mid p_{1}p_{2}\cdots p_{k}$. In
other words, $p\mid n-1$ (since $n-1=p_{1}p_{2}\cdots p_{k}$). In other words,
$n\equiv1\operatorname{mod}p$.

But $p\mid n$, so $n\equiv0\operatorname{mod}p$. Hence, $0\equiv
n\equiv1\operatorname{mod}p$, so that $p\mid1-0=1$. This is absurd.]

In other words, the prime $p$ is distinct from each of $p_{1},p_{2}%
,\ldots,p_{k}$.
\end{proof}

\section{Euler's totient function ($\phi$-function)}

Recall $\mathbb{P}=\left\{  1,2,3,\ldots\right\}  $.

\begin{definition}
Define a function $\phi:\mathbb{P}\rightarrow\mathbb{N}$ as follows:

For each $n\in\mathbb{P}$, we let%
\begin{align*}
\phi\left(  n\right)    & =\left(  \text{the number of all }i\in\left\{
1,2,\ldots,n\right\}  \text{ coprime to }n\right)  \\
...
\begin{exam}
\textbf{(a)} We have $\phi\left(  12\right)  =4$, since the $i\in\left\{
1,2,\ldots,12\right\}  $ coprime to $12$ are $1,5,7,11$. (See%
\[
1,\text{2},\text{3},\text{4},5,\text{6},7,\text{8},\text{9},\text{10}%
,11,\text{12.}%
\]
)

\textbf{(b)} We have $\phi\left(  13\right)  =12$, since the $i\in\left\{
1,2,\ldots,13\right\}  $ coprime to $13$ are all of $1,2,\ldots,12$.

\textbf{(c)} We have $\phi\left(  14\right)  =6$, since%
\[
1,\text{2},3,\text{4},5,\text{6},\text{7},\text{8},9,\text{10},11,\text{12}%
,13,\text{14}.
\]
...
\begin{proposition}
Let $p$ be a prime. Then, $\phi\left(  p\right)  =p-1$.
\end{proposition}

\begin{exercise}
Let $p$ be a prime. Let $k$ be a positive integer. Then, $\phi\left(
p^{k}\right)  =\left(  p-1\right)  p^{k-1}.$
\end{exercise}

\begin{theorem}
Let $n$ be a positive integer. Then,%
\begin{align*}
\phi\left(  n\right)    & =\prod_{p\text{ prime}}\left(  \left(  p-1\right)
p^{v_{p}\left(  n\right)  -1}\right)  \\
& =n\cdot\prod_{\substack{p\text{ prime;}\\p\mid n}}\left(  1-\dfrac{1}%
{p}\right)  .
\end{align*}
...
Let $n$ be a positive integer. Then,%
\[
\sum\limits_{d\mid n}\phi\left(  d\right)  =n.
\]
Here and in the following, $\sum\limits_{d\mid n}$ means \textquotedblleft sum
over all positive divisors $d$ of $n$\textquotedblright.
\end{theorem}

\begin{example}
For $n=12$, this is saying that%
\[
...
\begin{proof}
[Proof of Theorem.] Fix a positive divisor $d$ of $n$. We claim that%
\begin{align*}
& \phi\left(  d\right)  \\
& =\left(  \text{the number of }i\in\left\{  1,2,\ldots,n\right\}  \text{ such
that }\gcd\left(  i,n\right)  =n/d\right)  .
\end{align*}


[\textit{Proof of Claim:} The definition of $\phi$ yields%
\begin{align*}
& \phi\left(  d\right)  \\
& =\left\vert \left\{  i\in\left\{  1,2,\ldots,d\right\}  \ \mid\ i\perp
d\right\}  \right\vert \\
& =\left\vert \left\{  j\in\left\{  1,2,\ldots,d\right\}  \ \mid\ j\perp
d\right\}  \right\vert \\
& =\left\vert \left\{  j\in\left\{  1,2,\ldots,d\right\}  \ \mid\ \gcd\left(
j,d\right)  =1\right\}  \right\vert .
\end{align*}
But there is a map%
\begin{align*}
& \left\{  j\in\left\{  1,2,\ldots,d\right\}  \ \mid\ \gcd\left(  j,d\right)
=1\right\}  \\
& \rightarrow\left\{  i\in\left\{  1,2,\ldots,n\right\}  \ \mid\ \gcd\left(
i,n\right)  =n/d\right\}
\end{align*}
sending each $j$ to $\left(  n/d\right)  \cdot j$. To prove this, recall that
$\gcd\left(  sa,sb\right)  =\left\vert s\right\vert \cdot\gcd\left(
a,b\right)  $. This map is a bijection, since it has an inverse map sending
each $i$ to $i/\left(  n/d\right)  $. So we have%
\begin{align*}
& \left\vert \left\{  j\in\left\{  1,2,\ldots,d\right\}  \ \mid\ \gcd\left(
j,d\right)  =1\right\}  \right\vert \\
& =\left\vert \left\{  i\in\left\{  1,2,\ldots,n\right\}  \ \mid\ \gcd\left(
i,n\right)  =n/d\right\}  \right\vert \\
& =\left(  \text{the number of }i\in\left\{  1,2,\ldots,n\right\}  \text{ such
that }\gcd\left(  i,n\right)  =n/d\right)  .
\end{align*}
...
1,2,\ldots,n\right\}  \text{ such that }\gcd\left(  i,n\right)  =n/d\right)
}_{\substack{=\phi\left(  d\right)  \\\text{(by Claim)}}}\\
& \ \ \ \ \ \ \ \ \ \ \left(  \text{here, we have substituted }n/d\text{ for
}d\right)  \\
& =\sum_{d\mid n}\phi\left(  d\right)  .
\end{align*}

\end{proof}

\begin{theorem}
Let $m$ and $n$ be two coprime integers. Then, $\phi\left(  mn\right)

...
%TCIDATA{LastRevised=Friday, February 08, 2019 09:55:25}
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.coprime.combinek}.]TODO. (Induction on $k$,
using Theorem \ref{thm.ent.coprime.combine} and Theorem
\ref{thm.ent.coprime.ab-to-c}.)
\end{proof}

\subsubsection{\bigskip A bit more on coprime integers}
...
\begin{proof}
We have $\gcd\left(  a,c\right)  =1$. Thus, Bezout says there are $u,v$ such
that $1=ua+vc$.

We have $\gcd\left(  b,c\right)  =1$. Thus, Bezout says there are $x,y$ such
that $1=xb+yc$.

Now, multiplying $1=ua+vc$ with $1=xb+yc$, we get%
\begin{align*}
1  & =\left(  ua+vc\right)  \left(  xb+yc\right)  \\
& =uxab+uyac+vxcb+vycc\\
& =\left(  ux\right)  \left(  ab\right)  +\left(  uya+vxc+vyc\right)  c\\
& =\alpha\left(  ab\right)  +\beta c\ \ \ \ \ \ \ \ \ \ \text{for some }%
\alpha,\beta\in\mathbb{Z}.
\end{align*}
Thus, $\gcd\left(  ab,c\right)  \mid\alpha\left(  ab\right)  +\beta c=1$, so
that $\gcd\left(  ab,c\right)  =1$, so that $ab\perp c$.
\end{proof}

\begin{exercise}
Let $c\in\mathbb{Z}$. Let $a_{1},a_{2},\ldots,a_{k}$ be all coprime to $c$.
Prove that $a_{1}a_{2}\cdots a_{k}\perp c$.
\end{exercise}

\begin{proof}
Induction on $k$.

\textit{Base case:} $k=0$. Then, $a_{1}a_{2}\cdots a_{k}=\left(  \text{empty
product}\right)  =1\perp c$ since $\gcd\left(  1,c\right)  \mid1$.

\textit{Step:} Assume the exercise is solved for $k-1$. We want to solve it
for $k$.

By the induction hypothesis, we know that $a_{1}a_{2}\cdots a_{k-1}\perp c$.
And we know that $a_{k}\perp c$. Thus, $\left(  a_{1}a_{2}\cdots
a_{k-1}\right)  a_{k}\perp c$ (by the Theorem). In other words, $a_{1}%
a_{2}\cdots a_{k}\perp c$.
\end{proof}

\begin{exercise}
Let $c\in\mathbb{Z}$, and let $b_{1},b_{2},\ldots,b_{k}$ be mutually coprime
divisors of $c$. (\textquotedblleft Mutually coprime\textquotedblright\ means
$b_{i}\perp b_{j}$ for all $i\neq j$.) Prove that $b_{1}b_{2}\cdots b_{k}\mid
c$.
\end{exercise}
...
\textit{Base case:} $k=0$, easy.

\textit{Step:} Assume the exercise is solved for $k-1$. We want to solve it
for $k$.

By the induction hypothesis, we have $b_{1}b_{2}\cdots b_{k-1}\mid c$. And we
know that $b_{k}\mid c$. We want to get $\left(  b_{1}b_{2}\cdots
b_{k-1}\right)  b_{k}\mid c$. This will follow from Theorem 2.59 once we have
shown that $b_{1}b_{2}\cdots b_{k-1}\perp b_{k}$. But this follows from the
previous exercise, if we let $b_{1},b_{2},\ldots,b_{k-1}$ play the role of the
$a_{1},a_{2},\ldots,a_{k}$, and $b_{k}$ play the role of $c$.

So we have $\left(  b_{1}b_{2}\cdots b_{k-1}\right)  b_{k}\mid c$. In other
words, $b_{1}b_{2}\cdots b_{k}\mid c$.
\end{proof}

\begin{exercise}
Let $a,b$ be two coprime integers. Let $n,m\in\mathbb{N}$. Then, $a^{n}\perp
b^{m}$.
\end{exercise}

\begin{proof}
We have $a\perp b$. Thus, the second-to-last exercise yields
$\underbrace{aa\cdots a}_{n\text{ factors}}\perp b$. In other words,
$a^{n}\perp b$. In other words, $b\perp a^{n}$. Thus, the second-to-last
exercise yields $\underbrace{bb\cdots b}_{m\text{ factors}}\perp a^{n}$. In
other words, $b^{m}\perp a^{n}$. In other words, $a^{n}\perp b^{m}$.
\end{proof}
...
integer $c$ such that $\left(  b_{i}\mid c\text{ for all }i\right)  $. In
other words, it is an integer that is a multiple of each of $b_{1}%
,b_{2},\ldots,b_{k}$.

\textbf{(b)} The \textbf{lowest common multiple} (or \textbf{least common
...
a,b\right)  }$. Then, $c=a\cdot\underbrace{\dfrac{b}{\gcd\left(  a,b\right)
}}_{\in\mathbb{Z}}$, so that $c$ is an integer and a multiple of $a$.
Similarly, $c$ is a multiple of $b$ as well. Thus, $c$ is a common multiple of
$a$ and $b$. Hence, $\left\vert c\right\vert $ is a positive common multiple
of $a$ and $b$. Therefore, $\left\vert c\right\vert $ is $\geq$ to the
smallest positive common multiple of $a$ and $b$. In other words, $\left\vert
c\right\vert \geq\operatorname{lcm}\left(  a,b\right)  $.

Next, let me prove that $c\mid\operatorname{lcm}\left(  a,b\right)  $.

Let $\ell=\operatorname{lcm}\left(  a,b\right)  $. Thus, we need to prove
$c\mid\ell$.

In other words, we need to prove $\dfrac{ab}{\gcd\left(  a,b\right)  }\mid
\ell$.

Equivalently, we need to prove $ab\mid\gcd\left(  a,b\right)  \cdot\ell$.



This follows from Theorem \ref{thm.ent.gcd.combine}, applied to $c=\ell$
(because $\ell$ is a common multiple of $a$ and $b$).

So we have $c\mid\operatorname{lcm}\left(  a,b\right)  $. Thus, $\left\vert
c\right\vert \leq\operatorname{lcm}\left(  a,b\right)  $ (since the lcm is
positive). Combined with $\left\vert c\right\vert \geq\operatorname{lcm}%
\left(  a,b\right)  $, this yields $\left\vert c\right\vert
=\operatorname{lcm}\left(  a,b\right)  $. Thus,%
\[
...
\]
so that $\gcd\left(  a,b\right)  \cdot\operatorname{lcm}\left(  a,b\right)
...
$\operatorname*{Mul}$ means \textquotedblleft common multiples
of\textquotedblright.
\end{theorem}
...
\begin{example}
Assume we want to find an $x\in\mathbb{Z}$ such that%
\[
...
}\equiv1\operatorname{mod}m$ (since $m$ and $n$ play symmetric roles in the theorem).
...
\]
and similarly $x\equiv b\operatorname{mod}n$. So we have found our $x$.
...
these combine to $mn\mid x_{1}-x_{2}$ (by Theorem 2.59). In other words,
$x_{1}\equiv x_{2}\operatorname{mod}mn$.
\end{proof}

The theorem we just proved is (the simplest form of) the \textit{Chinese
Remainder Theorem} (Sunzi, 3rd century AD).

There is also a version for multiple integers:
...
just called \textbf{a prime}.
\end{definition}

Note that $p>1$ is required here, so $1$ does not count as prime.
...
,\text{28},29,\text{30}%
\end{align*}
The red numbers remaining are the primes $\leq30$. This is the \textbf{sieve
of Eratosthenes}.
...
\begin{proof}
Let $i\in\left\{  1,2,\ldots,p-1\right\}  $. We must prove that $\gcd\left(
i,p\right)  =1$.

We have $\gcd\left(  i,p\right)  \mid p$, so $\gcd\left(  i,p\right)  $ is a
positive divisor of $p$, so it is either $1$ or $p$ (since $p$ is prime).
Also, $\gcd\left(  i,p\right)  $ is a positive divisor of $i$, so that
$\gcd\left(  i,p\right)  \leq i<p$. Hence, $\gcd\left(  i,p\right)  $ cannot
be $p$. Hence, $\gcd\left(  i,p\right)  =1$, and we are done.
\end{proof}
...
\begin{proof}
Exercise. (This will be in the notes.)
\end{proof}

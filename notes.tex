...
%TCIDATA{LastRevised=Saturday, January 26, 2019 00:21:20}
...
\begin{definition}
Let $a$ and $b$ be two integers. We say that $a\mid b$ (or \textquotedblleft%
$a$ \textit{divides} $b$\textquotedblright\ or \textquotedblleft$b$ is
\textit{divisible by }$a$\textquotedblright\ or \textquotedblleft$b$ is a
\textit{multiple} of $a$\textquotedblright) if there exists an integer $c$
such that $b=ac$.
\end{definition}

\begin{example}
\textbf{(a)} We have $4\mid12$, since $12=4\cdot3$.

\textbf{(b)} We have $a\mid0$ for any $a\in\mathbb{Z}$, since $0=a\cdot0$.

\textbf{(c)} We have $0\mid b$ only when $b=0$, since $0\mid b$ implies
$b=0c=0$.

\textbf{(d)} We have $a\mid a$ for any $a\in\mathbb{Z}$.

\textbf{(e)} We have $1\mid b$ for each $b\in\mathbb{Z}$, since $b=1b$.
\end{example}
...
\begin{proposition}
\textbf{(a)} We have $a\mid a$ for every $a\in\mathbb{Z}$.
...
\begin{definition}
Let $n,a,b\in\mathbb{Z}$. We say that $a$ \textit{is congruent to }$b$
\textit{modulo }$n$ (or, short, \textquotedblleft$a\equiv b\operatorname{mod}%
n$\textquotedblright) if and only if $n\mid a-b$.
\end{definition}

\begin{example}
\textbf{(a)} Is $3\equiv7\operatorname{mod}2$ ? Yes, since $2\mid3-7=-4$.

\textbf{(b)} Is $3\equiv6\operatorname{mod}2$ ? No, since $2\nmid3-6=-3$.

\textbf{(c)} We have $a\equiv b\operatorname{mod}0$ if and only if $a=b$.

\textbf{(d)} We have $a\equiv b\operatorname{mod}1$ always, since $1$ divides everything.
\end{example}

Note that being congruent modulo $2$ means having the same parity: i.e., two
even numbers will be congruent modulo $2$, and two odd numbers will be, but
not an even and an odd.

...
%TCIDATA{LastRevised=Thursday, February 14, 2019 01:53:13}
...
reveals that%
\begin{equation}
a_{n}=11a_{n-1}-18a_{n-2}\ \ \ \ \ \ \ \ \ \ \text{for each }n\geq
2.\label{sol.ent.mod.substitutivity.7div.rec}%
\end{equation}
...
$m$. Thus, $m=\left\vert h\right\vert $. From $N=gh$, we obtain%
\begin{align}
\left\vert N\right\vert  & =\left\vert gh\right\vert =\underbrace{\left\vert
g\right\vert }_{\substack{=g\\\text{(since }g\text{ is positive)}}%
}\cdot\underbrace{\left\vert h\right\vert }_{=m}\ \ \ \ \ \ \ \ \ \ \left(
\text{by (\ref{eq.ent.div.abs(xy)})}\right)  \nonumber\\
...
wa$. Similarly, $N\mid wb$ and $N\mid wc$. Thus, $\left(  N\mid wa\text{ and
}N\mid wb\text{ and }N\mid wc\right)  $. In other words, $N$ is a common
divisor of $wa,wb,wc$. In other words, $N\in\operatorname*{Div}\left(
wa,wb,wc\right)  $.

But Theorem \ref{thm.ent.gcd.uniprop-mul} \textbf{(a)} (applied to $3$,
$\left(  wa,wb,wc\right)  $ and $N$ instead of $k$, $\left(  b_{1}%
,b_{2},\ldots,b_{k}\right)  $ and $m$) shows that we have the following
equivalence:%
\[
\left(  N\mid wa\text{ and }N\mid wb\text{ and }N\mid wc\right)
\ \Longleftrightarrow\ \left(  N\mid\gcd\left(  wa,wb,wc\right)  \right)  .
\]
Thus, we have $N\mid\gcd\left(  wa,wb,wc\right)  $ (since we have $\left(
N\mid wa\text{ and }N\mid wb\text{ and }N\mid wc\right)  $).

Exercise \ref{exe.ent.gcd.sa,sb,sc} (applied to $s=w$) yields $\gcd\left(
wa,wb,wc\right)  =\underbrace{\left\vert w\right\vert }%
_{\substack{=w\\\text{(since }w\text{ is positive)}}}\underbrace{\gcd\left(
a,b,c\right)  }_{=g}=wg$. Hence,%
\[
N\mid\gcd\left(  wa,wb,wc\right)  =wg.
\]
Hence, $hg=gh=N\mid wg$.

But $g\neq0$ (since $gh=N\neq0$). Hence, Exercise \ref{exe.ent.div.acbc}
(applied to $h$, $w$ and $g$ instead of $a$, $b$ and $c$) shows that $h\mid w$
holds if and only if $hg\mid wg$. Hence, $h\mid w$ holds (since $hg\mid wg$).
Hence, Proposition \ref{prop.ent.div.1} \textbf{(b)} (applied to $h$ and $w$
instead of $a$ and $b$) yields $\left\vert h\right\vert \leq\left\vert
w\right\vert $ (since $w\neq0$). But $\left\vert w\right\vert =w$ (since $w$
is positive). Thus, $w=\left\vert w\right\vert \geq\left\vert h\right\vert =m$.
...
\[
\left\vert N\right\vert =\underbrace{g}_{=\gcd\left(  a,b,c\right)
}\underbrace{m}_{=\operatorname{lcm}\left(  x,y,z\right)  }=\gcd\left(
a,b,c\right)  \cdot\operatorname{lcm}\left(  x,y,z\right)  .
\]
This proves Claim 1.
...
\begin{proof}
[Proof of Proposition \ref{prop.ent.primes.gcd}.]It is easy to see that the
infinite products $\prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(
n\right)  ,v_{p}\left(  m\right)  \right\}  }$ and $\prod_{p\text{ prime}%
}p^{\max\left\{  v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }$
are well-defined\footnote{\textit{Proof.} Let $p$ be a prime such that
$p>\max\left\{  \left\vert m\right\vert ,\left\vert n\right\vert \right\}  $.
Thus, $p>\max\left\{  \left\vert m\right\vert ,\left\vert n\right\vert
\right\}  \geq\left\vert m\right\vert $ and therefore $v_{p}\left(  m\right)
=0$ (by Lemma \ref{lem.ent.primes.vpn=0} \textbf{(a)}, applied to $m$ instead
of $n$). Similarly, $v_{p}\left(  n\right)  =0$. Hence, $\max\left\{
\underbrace{v_{p}\left(  n\right)  }_{=0},\underbrace{v_{p}\left(  m\right)
}_{=0}\right\}  =\max\left\{  0,0\right\}  =0$ and therefore $p^{\max\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }=p^{0}=1$.
\par
Now, forget that we fixed $p$. We thus have proven that every prime
$p>\max\left\{  \left\vert m\right\vert ,\left\vert n\right\vert \right\}  $
...
Recall $n=\prod\limits_{p\text{ prime}}p^{v_{p}\left(  n\right)  }$. But each
prime $p$ satisfies $\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  \leq v_{p}\left(  n\right)  $ and thus $p^{\min\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }\mid p^{v_{p}\left(
n\right)  }$. So $\prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(
n\right)  ,v_{p}\left(  m\right)  \right\}  }\mid\prod\limits_{p\text{ prime}%
}p^{v_{p}\left(  n\right)  }$. In other words, $g\mid n$ (since $g=\prod
\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  }$ and $n=\prod\limits_{p\text{ prime}}p^{v_{p}\left(
n\right)  }$). Similarly, $g\mid m$. Thus, $g$ is a common divisor of $n$ and
$m$.

Hence, Lemma \ref{lem.ent.gcd.uniprop} yields $g\mid\gcd\left(  n,m\right)
=h$.

On the other hand, fix any prime $p$. Then, $h\mid n$, and thus $v_{p}\left(
h\right)  \leq v_{p}\left(  n\right)  $ (by Proposition
\ref{prop.ent.primes.n|m}). Similarly, $v_{p}\left(  h\right)  \leq
v_{p}\left(  m\right)  $. Combining these two inequalities, we obtain%
\[
...
The proof of the second equation is entirely symmetric: just flip all
divisibilities and inequalities and replace \textquotedblleft$\min
$\textquotedblright\ by \textquotedblleft$\max$\textquotedblright.
\end{proof}

...
%TCIDATA{LastRevised=Friday, February 08, 2019 01:20:13}
...
\ref{thm.ent.gcd.uniprop-mul} separately, let us prove them as a package.
...
Statement 2 \textbf{(b)} is a restatement of Statement 2 \textbf{(a)}.

Statement 2 \textbf{(c)} is a restatement of Statement 2 \textbf{(b)}.
...
In other words, Theorem \ref{thm.ent.gcd.bezout-mul} states that if
$b_{1},b_{2},\ldots,b_{k}$ are integers, then $\gcd\left(  b_{1},b_{2}%
,\ldots,b_{k}\right)  $ is a $\mathbb{Z}$-linear combination of $b_{1}%
,b_{2},\ldots,b_{k}$ (that is, a linear combination of $b_{1},b_{2}%
,\ldots,b_{k}$ with integer coefficients).
...
\textit{Induction base:} Theorem \ref{thm.ent.gcd.bezout-mul} holds for $k=0$.
(This is fairly trivial: $\gcd\left(  {}\right)  =0$ is a linear combination
of an empty list of numbers.)

\textit{Induction step:} Let $\ell$ be a positive integer. Assume that Theorem
\ref{thm.ent.gcd.bezout-mul} holds for $k=\ell-1$. We must prove that Theorem
\ref{thm.ent.gcd.bezout-mul} holds for $k=\ell$.

We have assumed that Theorem \ref{thm.ent.gcd.bezout-mul} holds for $k=\ell
-1$. In other words, the following statement holds:
...
}\right)  =u\gcd\left(  b_{1},b_{2},\ldots,b_{\ell-1}\right)  +vb_{\ell}.
\]
...
\gcd\left(  b_{1},b_{2},\ldots,b_{\ell}\right)   &  =\gcd\left(  \gcd\left(
b_{1},b_{2},\ldots,b_{\ell-1}\right)  ,b_{\ell}\right) \\
&  =u\underbrace{\gcd\left(  b_{1},b_{2},\ldots,b_{\ell-1}\right)  }%
_{=y_{1}b_{1}+y_{2}b_{2}+\cdots+y_{\ell-1}b_{\ell-1}}+vb_{\ell}\\
&  =u\left(  y_{1}b_{1}+y_{2}b_{2}+\cdots+y_{\ell-1}b_{\ell-1}\right)
+vb_{\ell}\\
&  =uy_{1}b_{1}+uy_{2}b_{2}+\cdots+uy_{\ell-1}b_{\ell-1}+vb_{\ell}.
\end{align*}
This is a linear combination of the $b_{1},b_{2},\ldots,b_{\ell}$. So
Statement 2 is proven.

In other words, we have proven that Theorem \ref{thm.ent.gcd.bezout-mul} holds
for $k=\ell$. This completes the induction step. Thus, Theorem
...
\begin{proof}
[Proof of Theorem \ref{thm.ent.gcd.split}.]TODO! Prove left$\mid$right and
right$\mid$left. Better, proev that the same $m$'s divide both left and right.
\end{proof}
...
using Theorem \ref{thm.ent.coprime.ab-to-c}.)
\end{proof}

\begin{exercise}
\label{exe.ent.coprime.combinek}Let $c\in\mathbb{Z}$, and let $b_{1}%
,b_{2},\ldots,b_{k}$ be integers that are mutually coprime (i.e., we have

...
%TCIDATA{LastRevised=Monday, February 11, 2019 12:17:24}
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.primes.pab-conv}.]TODO.
\end{proof}
...
\begin{proof}
[Proof of Lemma \ref{lem.ent.prime.vp-wd}.]TODO: $p^{m}\mid n$ implies
$\left\vert n\right\vert \geq p^{m}$.
\end{proof}
...
(called the $p$\textit{-valuation} or the $p$\textit{-adic valuation} of $n$)
denote the largest $m\in\mathbb{N}$ such that $p^{m}\mid n$. This is
well-defined, by Lemma \ref{lem.ent.prime.vp-wd}.

\textbf{(b)} Set $v_{p}\left(  0\right)  =\infty$, where $\infty$ is a symbol
which we take to satisfy%
\begin{align*}
\infty+k &  =\infty\ \ \ \ \ \ \ \ \ \ \text{for all integers }k;\\
\infty+\infty &  =\infty;\\
k &  <\infty\ \ \ \ \ \ \ \ \ \ \text{for all integers }k;\\
\min\left(  S\cup\left\{  \infty\right\}  \right)   &  =\min
S\ \ \ \ \ \ \ \ \ \ \text{for any }S\subseteq\mathbb{Z}\text{ if }%
S\neq\varnothing;\\
\max\left(  S\cup\left\{  \infty\right\}  \right)   &  =\infty
\ \ \ \ \ \ \ \ \ \ \text{for any }S\subseteq\mathbb{Z}.
\end{align*}
(Note, however, that $\infty-\infty$ is not defined. Hence, $\infty$ is not
really a way to \textquotedblleft extend\textquotedblright\ our number system.)
\end{definition}
...
$\Longleftarrow:$ Assume $v_{p}\left(  n\right)  \geq i$. Hence, $i\leq
v_{p}\left(  n\right)  $, so that $p^{i}\mid p^{v_{p}\left(  n\right)  }$.

Recall that $v_{p}\left(  n\right)  $ is the largest $m\in\mathbb{N}$ such
that $p^{m}\mid n$. In particular, $p^{v_{p}\left(  n\right)  }\mid n$.
...
\begin{proof}
\textbf{(a)} WLOG assume that $a,b$ are nonzero (since otherwise, we are just
saying $\infty=\infty+\left(  \text{something}\right)  $).
...
by $p$. Thus, $x\perp p$ (by Proposition \ref{prop.ent.primes.div-or-coprime}).

Similarly, $y:=b/p^{v_{p}\left(  b\right)  }$ is an integer and $y\perp p$.

From $x\perp p$ and $y\perp p$, we obtain $xy\perp p$ (by Theorem
...
\begin{itemize}
\item $\gcd$ and $\operatorname{lcm}$ via canonical factorization

\item coprimality means not having any primes in common

\item there are $\infty$ many primes

\item define $\phi\left(  n\right)  $

\item examples

\item prove $\sum_{d\mid n}\phi\left(  d\right)  =n$
\end{itemize}

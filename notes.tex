...
%TCIDATA{LastRevised=Friday, February 01, 2019 22:59:07}
...
\begin{fineprint}
\begin{proof}[Solution to Exercise \ref{exe.ent.mod.basics.k-power}.] We want to prove
that
\begin{equation}
...
We have $u=qn+\underbrace{r}_{=u\%n=0}=qn$. Thus, $n\mid u$. This proves the
\textquotedblleft$\Longleftarrow$\textquotedblright\ implication of Corollary
\ref{cor.ent.quo-rem.remmod} \textbf{(b)}.
...
\textbf{(h)} Prove that $u^{2}\equiv u\operatorname{mod}2$.
\end{exercise}
...
Be aware that some books have a mildly different definition of
...
\textbf{(a)}); thus, the positive divisors are usually the only thing one is 
...
\begin{example}
The common divisors of $6$ and $8$ are $-2,-1,1,2$. In order to see this, just
observe that the divisors of $6$ are $-6,-3,-2,-1,1,2,3,6$, whereas the
divisors of $8$ are $-8,-4,-2,-1,1,2,4,8$; now you can find the common
divisors of $6$ and $8$ by taking the numbers common to these two lists.
\end{example}
...
\begin{proof}
[Proof of Proposition \ref{prop.ent.Div.fin}.] TODO.
\end{proof}

\begin{itemize}
\item Greatest common divisor. (Also set $\gcd\left(  0,0\right)  =0$.)

\item $\gcd\left(  a,ua+b\right)  =\gcd\left(  a,b\right)  $, or equivalently
$\gcd\left(  a,b\right)  =\gcd\left(  a,c\right)  $ if $b\equiv
c\operatorname{mod}a$.

\item $\gcd\left(  a,b\right)  =\gcd\left(  b,a\right)  $.

\item Euclidean algorithm.

\item Bezout for $a,b\geq0$, proven by strong ind/$a+b$.

\item Bezout for arbitrary $a,b$.

\item \textquotedblleft$\mathbb{Z}$-linear combination\textquotedblright.

\item corollary $m\mid a,\ m\mid b\ \Longrightarrow\ m\mid\gcd\left(
a,b\right)  $.

\item coprimality.

\item notation $a\perp b$.

\item corollary $ab\mid c,\ a\perp c\ \Longrightarrow\ b\mid c$.

\item corollary: existence of modular inverses.

\item $s\gcd\left(  a,b\right)  =\gcd\left(  sa,sb\right)  $.
...
is involved in our expressions. For example, we hasve $6\equiv

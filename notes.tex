...
%TCIDATA{LastRevised=Tuesday, February 19, 2019 00:17:44}
...
Theorem \ref{thm.ent.gcd.bezout} says that if $a$ and $b$ are two
integers, then $\gcd\tup{a, b}$ is a $\ZZ$-linear combination of $a$ and
$b$.
Note the indefinite article ``a'' here: There are (usually) many
$\ZZ$-linear combinations of $a$ and $b$, but only one gcd.
It is definitely not true that every $\ZZ$-linear combination of $a$
and $b$ must be $\gcd\tup{a, b}$.
However, all these $\ZZ$-linear combinations are \textbf{multiples} of
the gcd, as the following (simple) proposition says:

\begin{proposition}
\label{prop.ent.gcd.bezout-conv}Let $a$ and $b$ be two integers. Then, any
integers $x$ and $y$ satisfy $\gcd\tup{a, b} \mid xa + yb$.
\end{proposition}

\begin{proof}[Proof of Proposition \ref{prop.ent.gcd.bezout-conv}.]
Let $x$ and $y$ be integers.
Let $g = \gcd\tup{a, b}$.
Thus, $g = \gcd\tup{a, b} \mid a$ (by
Proposition \ref{prop.ent.gcd.props1} \textbf{(f)}).
Hence, $g \mid a \mid xa$ (since $xa = ax$).
In other words, $xa \equiv 0 \mod g$.
Similarly, $yb \equiv 0 \mod g$.
Adding these two congruences together, we obtain
$xa + yb \equiv 0 + 0 = 0 \mod g$.
In other words, $g \mid xa + yb$.
In other words, $\gcd\tup{a, b} \mid xa + yb$
(since $g = \gcd\tup{a, b}$).
This proves Proposition \ref{prop.ent.gcd.bezout-conv}.
\end{proof}

A similar proposition holds for $\ZZ$-linear combinations of
any number of integers $b_1, b_2, \ldots, b_k$.
...
\begin{exercise} \label{exe.ent.coprime.bezout-conv}
Let $a, b, x, y$ be integers such that $xa + yb = 1$.
Prove that $a \perp b$.
\end{exercise}

\begin{fineprint}
 \begin{proof}[Solution to Exercise \ref{exe.ent.coprime.bezout-conv}.]
  Proposition \ref{prop.ent.gcd.bezout-conv} yields
  $\gcd\tup{a, b} \mid xa + yb = 1$.
  Proposition \ref{prop.ent.div.1} \textbf{(b)} (applied
  to $\gcd\tup{a, b}$ and $1$ instead of $a$ and $b$)
  thus yields $\abs{\gcd\tup{a, b}} \leq \abs{1} = 1$.
  But $\abs{\gcd\tup{a, b}} = \gcd\tup{a, b}$ (since
  $\gcd\tup{a, b}$ is a nonnegative integer). Hence,
  $\gcd\tup{a, b} = \abs{\gcd\tup{a, b}} \leq 1$.
  Thus, $\gcd\tup{a, b}$ is either $0$ or $1$ (since
  $\gcd\tup{a, b}$ is a nonnegative integer).

  If we had $\gcd\tup{a, b} = 0$, then we would have
  $0 = \gcd\tup{a, b} \mid 1$, which is absurd (since
  we don't have $0 \mid 1$). Thus, we cannot have
  $\gcd\tup{a, b} = 0$. Hence, $\gcd\tup{a, b}$ must
  equal $1$ (since we know that $\gcd\tup{a, b}$ is
  either $0$ or $1$).
  In other words, $\gcd\tup{a, b} = 1$.
  In other words, $a$ is coprime to $b$.
  In other words, $a \perp b$.
  This solves Exercise \ref{exe.ent.coprime.bezout-conv}.
 \end{proof}
\end{fineprint}
...
\begin{proof}[Solution to Exercise \ref{exe.ent.euler345} (sketched).] The last
...
\begin{proof}[First proof of Lemma \ref{lem.ent.euler.phi0} (sketched).]
If $n=1$, then this lemma can easily be proven by hand. Thus, WLOG
assume that $n \neq 1$. Hence, $n > 1$ (since $n$ is a positive integer).
Thus, neither $0$ nor $n$ is coprime to $n$ (since
...
these two elements belongs to any of these two sets\footnote{since neither
$0$ nor $n$ is coprime to $n$}), and therefore%
\begin{align*}
...
\ \ \ \ \ \ \ \ \ \  \tup{\text{by \eqref{eq.def.ent.phi.phi.1}}} .
\end{align*}
This proves Lemma \ref{lem.ent.euler.phi0}.
\end{proof}

\begin{proof}[Second proof of Lemma \ref{lem.ent.euler.phi0}.]
We have $n \mid n$ and thus $n \equiv 0 \mod n$. Hence,
Proposition \ref{prop.ent.gcd.props1} \textbf{(d)} (applied to
$a = n$, $b = n$ and $c = 0$) yields that
$\gcd\tup{n, n} = \gcd\tup{n, 0} = \gcd\tup{0, n}$
(by Proposition \ref{prop.ent.gcd.props1} \textbf{(b)}).
Hence, $\gcd\tup{0, n} = 1$ holds if and only if $\gcd\tup{n, n} = 1$.
In other words, the number $0$ is coprime to $n$ if and only if $n$ is
coprime to $n$.
Hence, if we remove $n$ from the set
$\left\{  1,2,\ldots,n\right\}  $ and add $0$ instead
(so that our set becomes $\set{0, 1, \ldots, n-1}$), then the
number of elements
coprime to $n$ in that set does not change.
In other words,
\begin{align*}
&  \tup{ \text{the number of all }  i\in\left\{  0,1,\ldots,n-1\right\}  \text{ that are coprime to }
n } \\
&  =
  \tup{ \text{the number of all }  i\in\left\{  1,2,\ldots,n\right\}  \text{ that are coprime to }
n } \\
&= \left\vert \left\{  i\in\left\{  1,2,\ldots,n\right\}  \ \mid\ i\perp
n\right\}  \right\vert =\phi\left(  n\right)
\ \ \ \ \ \ \ \ \ \  \tup{\text{by \eqref{eq.def.ent.phi.phi.1}}} .
...
\begin{align*}
 ...
 &= \tup{ \text{the number of all }  i\in\left\{  0,1,\ldots,n-1\right\}  \text{ that are coprime to }
n } \\
 &= \left\vert \left\{  i\in\left\{  0,1,\ldots
,n-1\right\}  \ \mid\ i\perp n\right\}  \right\vert .
\end{align*}
...
\begin{noncompile}
We will also need a fact about modular inverses, which we state
as an exercise. Namely, recall that Theorem \ref{thm.ent.coprime.modinv}
\textbf{(b)} guarantees the existence of a modular inverse for an
integer $a$ modulo an integer $n$ as soon as $a$ and $n$ are coprime.
We can slightly improve this to guarantee a modular inverse that
belongs to $\set{0, 1, \ldots, n-1}$ as long as $n$ is a positive
integer:

\begin{exercise}
\label{exe.ent.coprime.modinv-in-set}Let $a$ be an integer,
and $n$ be a positive integer
such that $a \perp n$.
Then, there exists an $a^{\prime}\in\set{0,1,\ldots,n-1}$
such that $aa^{\prime}\equiv1\operatorname{mod}n$.
\end{exercise}

\begin{fineprint}
 \begin{proof}[Solution to Exercise \ref{exe.ent.coprime.modinv-in-set}.]
  Theorem \ref{thm.ent.coprime.modinv} \textbf{(b)} shows that
  there exists an $a^{\prime}\in\mathbb{Z}$
  such that $aa^{\prime}\equiv1\operatorname{mod}n$.
  Consider this $a^{\prime}$, and denote it by $u$.
  Thus, $u$ is an element of $\ZZ$ and satisfies
  $au \equiv 1 \mod n$.
  Corollary \ref{cor.ent.quo-rem.remmod} \textbf{(a)} yields
  that $u\%n\in\left\{  0,1,\ldots,n-1\right\}  $ and $u\%n\equiv
  u\operatorname{mod}n$.
  Multiplying the congruences $a \equiv a \mod n$
  and $u \% n \equiv u \mod n$ together,
  we find $a \tup{u\%n} \equiv au \equiv 1 \mod n$.
  Hence, there exists an $a^{\prime}\in\set{0,1,\ldots,n-1}$
  such that $aa^{\prime}\equiv1\operatorname{mod}n$
  (namely, $a^{\prime} = u \% n$).
  This solves Exercise \ref{exe.ent.coprime.modinv-in-set}.
 \end{proof}
...
Now, $z=\prod_{i\in C}i$. Then, $z\perp n$ (by Exercise \ref{exe.ent.coprime.ab-to-cI},
since each $i \in C$ satisfies $i \perp n$).
...
[\textit{Proof:} Let $i \in C$. We shall prove that $i \in f\tup{C}$.

Indeed, $i \in C$. Thus, $i \in \set{0, 1, \ldots, n-1}$
(by the definition of $C$).

But Exercise \ref{exe.ent.coprime.modinv-in-set} shows that
there exists an $a^{\prime}\in\set{0,1,\ldots,n-1}$
such that $aa^{\prime}\equiv1\operatorname{mod}n$.
Consider this $a^{\prime}$, and denote it by $u$.
Thus, $u$ is an element of $\set{0, 1, \ldots, n-1}$ and satisfies
$au \equiv 1 \mod n$.
From $ua = au \equiv 1 \mod n$,
we conclude that there exists an $u^{\prime} \in \mathbb{Z}$ such
that $uu^{\prime} \equiv 1 \mod n$ (namely, $u^{\prime} = a$).
Hence, Theorem \ref{thm.ent.coprime.modinv}
\textbf{(c)} (applied to $u$ and $u^{\prime}$ instead of $a$ and
$a^{\prime}$) shows that $u \perp n$.
From $u \in \set{0, 1, \ldots, n-1}$ and $u \perp n$, we conclude
that $u \in C$ (by the definition of $C$).
Multiplying the two congruences $au \equiv 1 \mod n$ and
$i \equiv i \mod n$, we obtain
$aui \equiv 1i = i \mod n$.
But Corollary \ref{cor.ent.quo-rem.remmod} \textbf{(c)} says:
If $c\in\left\{  0,1,\ldots,n-1\right\}  $ is such that $c\equiv
u\operatorname{mod}n$, then $c=u\%n$.
[...]
]
\end{noncompile}
...
i\right)  $ for $i$ in the product $\prod\limits_{i\in C}i$. So%
\begin{align*}
\prod_{i\in C}i  &  =\prod_{i\in C}\underbrace{f\left(  i\right)
}_{\substack{=\left(  ai\right)  \%n\\\equiv ai\operatorname{mod}n}%
}\equiv\prod_{i\in C}\left(  ai\right)  \ \ \ \ \ \ \ \ \ \ \left(  \text{by
(\ref{eq.exe.ent.mod.k-sum.b})}\right) \\
&  =a^{\left\vert C\right\vert }\prod_{i\in C}i=a^{\phi\left(  n\right)
}\prod_{i\in C}i\operatorname{mod}n\ \ \ \ \ \ \ \ \ \ \left(  \text{since
}\left\vert C\right\vert =\phi\left(  n\right)  \right)  .
\end{align*}
In other words, $z\equiv a^{\phi\left(  n\right)  }z\operatorname{mod}n$
(since $z=\prod\limits_{i\in C}i$). Lemma \ref{lem.ent.coprime.cancel} lets us
...
\begin{proof}
(Proof sketch. For details, see \cite[Exercise 3.4]{detnotes}, which proves
the Theorem in the case $k\in\mathbb{N}$. The remaining case $k\notin%
\mathbb{N}$ is obvious, because in that case the theorem simply says $0=0$.)

We proceed by induction on $n$:

\textit{Induction base:} $n=0$. Then, $\dbinom{n}{k}=\dbinom{0}{k}=%
\begin{cases}
1, & \text{if }k=0;\\
0, & \text{if }k\neq0
\end{cases}
$ (follows from the definition). On the other hand, the set $N$ is empty
(since $\left\vert N\right\vert =n=0$). Thus, its only subset is $\varnothing
$, which is a $0$-element subset. Hence, the number of $k$-element subsets of
$N$ is $%
\begin{cases}
1, & \text{if }k=0;\\
0, & \text{if }k\neq0
...
\textit{Induction step:} Let $n>0$. Assume (as the induction hypothesis) that
the theorem holds for $n-1$ instead of $n$.

Now, let $N$ be an $n$-element set. Pick any $a\in N$ (this exists because
$\left\vert N\right\vert =n>0$). Now, the $k$-element subsets of $N$ can be
classified into two types:

\begin{itemize}
\item Type 1: those that don't contain $a$;

\item Type 2: those that contain $a$.
\end{itemize}

The number of Type-1 subsets is $\dbinom{n-1}{k}$ (by the induction
hypothesis, since the Type-1 subsets are precisely the $k$-element subsets of
the $\left(  n-1\right)  $-element set $N\setminus\left\{  a\right\}  $).

The number of Type-2 subsets is $\dbinom{n-1}{k-1}$. (In fact, there is a
bijection%
\begin{align*}
\left\{  \left(  k-1\right)  \text{-element subsets of }N\setminus\left\{
a\right\}  \right\}   &  \rightarrow\left\{  \text{Type-2 }k\text{-element
subsets of }N\right\}  ,\\
S  &  \mapsto S\cup\left\{  a\right\}  .
\end{align*}
Hence,%
\begin{align*}
&  \left(  \text{the number of Type-2 }k\text{-element subsets of }N\right) \\
&  =\left(  \text{the number of }\left(  k-1\right)  \text{-element subsets of
}N\setminus\left\{  a\right\}  \right) \\
&  =\dbinom{n-1}{k-1}\ \ \ \ \ \ \ \ \ \ \left(  \text{by the induction
hypothesis}\right)  .
\end{align*}
)

Combining both counts, we conclude that%
\begin{align*}
&  \left(  \text{the number of }k\text{-element subsets of }N\right) \\
&  =\dbinom{n-1}{k}+\dbinom{n-1}{k-1}=\dbinom{n}{k}%
\end{align*}
(by the recurrence of the binomial coefficients). So the induction step is complete.
\end{proof}
...
\begin{proof}
If $n\geq0$, then this follows from the Corollary we just proved. Thus, WLOG
assume that $n<0$.

WLOG assume that $k\in\mathbb{N}$ (since otherwise, $\dbinom{n}{k}=0$).

The upper negation formula (applied to $-n$ instead of $n$) yields%
\[
\dbinom{n}{k}=\left(  -1\right)  ^{k}\underbrace{\dbinom{k-n-1}{k}%
}_{\substack{\text{a nonnegative integer}\\\text{(by Corollary, since
}k-n-1\in\mathbb{N}\\\text{(because }k\in\mathbb{N}\text{ and }-n-1\in
\mathbb{N}\text{))}}}.
\]
This is an integer.
\end{proof}

The Proposition allows us to study divisibilities and congruences between
binomial coefficients (and you have seen a few of them on HW1). The study of
binomial coefficients properly belongs into Combinatorics, but we will need
some of the most basic results in this class. Here are two of the main
identities for binomial coefficients:

\begin{theorem}
(The Binomial Formula) Let $x,y$ be any numbers. Let $n\in\mathbb{N}$. Then,%
\[
...
\begin{proof}
Straightforward induction on $n$. See \cite[Exercise 3.6]{detnotes} (or pretty
much any text on combinatorics) for details.
\end{proof}

\begin{theorem}
(Vandermonde convolution) Let $x,y\in\mathbb{Q}$ and $n\in\mathbb{N}$. Then,%
\[
...
\begin{proof}
[Proof outline.]The proof we are going to give is similar to the one given in
\cite[\S 3.3.3]{detnotes} (but, unlike the latter proof, we use polynomials in
$1$ variable only). See \cite[\S 3.3.2]{detnotes} for another (more boring and
tedious, but conceptually simpler proof).
...
$\dbinom{x+y}{n}$ (by the Combinatorial Interpretation), since $A$ is an
...
\begin{theorem}
(Babbage's congruence) If $a,b\in\mathbb{Z}$ and if $p$ is a prime,%
\[

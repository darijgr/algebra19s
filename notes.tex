...
%TCIDATA{LastRevised=Monday, February 04, 2019 10:15:00}
...
The common divisors of a single integers $b$ are merely the divisors of $b$.
Thus, $\operatorname*{Div}\left(  b\right)  $ is the set of all divisors of
$b$ whenever $b\in\mathbb{Z}$.

(Also, the common divisors of an empty list of integers are all integers,
because the requirement (\ref{eq.def.ent.Div.cond}) is vacuously true for
$k=0$. Thus, $\operatorname*{Div}\left(  {}\right)  =\mathbb{Z}$.)
...
words, $\operatorname*{Div}\left(  b_{1},b_{2},\ldots,b_{k}\right)
\subseteq\left\{  -\left\vert b_{j}\right\vert ,-\left\vert b_{j}\right\vert
+1,\ldots,\left\vert b_{j}\right\vert \right\}  \setminus\left\{  0\right\}
...
\item If $b_{1},b_{2},\ldots,b_{k}$ are not all $0$, then it is defined to be
$0$.
\end{itemize}
...
\begin{proof}
[Proof of Proposition \ref{prop.ent.gcd.props1}.]TODO
...
\ref{prop.ent.gcd.props1} \textbf{(b)}}\right)  \\
& =\gcd\left(  15,\underbrace{30\%15}_{=0}\right)  \ \ \ \ \ \ \ \ \ \ \left(
\text{by Proposition \ref{prop.ent.gcd.props1} \textbf{(e)}}\right)  \\
...
TODO: Define \textquotedblleft$\mathbb{Z}$-linear
combination\textquotedblright.
...
$a$ and $b$ are \textit{coprime} if and only if $\gcd\left(  a,b\right)  =1$.
\end{definition}

Instead of \textquotedblleft coprime\textquotedblright, some authors say
\textquotedblleft relatively prime\textquotedblright.

\begin{example}
\textbf{(a)} The numbers $2$ and $3$ are coprime, since $\gcd\left(
2,3\right)  =1$.

\textbf{(b)} More generally, for any integer $a$, the numbers $a$ and $a+1$
are coprime. To prove this, we note that%
\begin{align*}
...
a positive integer), which means that $a$ and $a+b$ are coprime.

\textbf{(c)} The numbers $6$ and $15$ are not coprime, since $\gcd\left(
6,15\right)  =3\neq1$.
\end{example}
...
\textquotedblleft$a\perp b$\textquotedblright\ to signify that $a$ and $b$ are coprime.
\end{definition}
...
divisibility $ab\mid c$ as long as this factor is coprime to $c$:

\begin{theorem}
\label{thm.ent.coprime.cancel}Let $a,b,c\in\mathbb{Z}$ satisfy $a\mid bc$ and
$b\perp c$. Then, $a\mid c$.
\end{theorem}

\begin{proof}
[Proof of Theorem \ref{thm.ent.coprime.cancel}.]We have $b\perp c$; in other
words, $b$ is coprime to $c$ (by Definition \ref{def.ent.coprime.perp}).

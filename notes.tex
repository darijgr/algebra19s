...
%TCIDATA{LastRevised=Friday, February 01, 2019 11:02:11}
...
integer function}. There is also a \textit{ceiling function}, which sends each
$x\in\mathbb{R}$ to the unique integer $n$ satisfying $n-1<x\leq n$; this
latter integer is called $\left\lceil x\right\rceil $. The two functions are
connected by the rule $\left\lceil x\right\rceil =-\left\lfloor
-x\right\rfloor $ (for all $x\in\mathbb{R}$).
...
\begin{itemize}
\item Define divisors of $a$.

\item How to find all positive divisors of $a$ (they are all $\leq\left\vert
a\right\vert $).

\item Common divisors.

\item Greatest common divisor. (Also set $\gcd\left(  0,0\right)  =0$.)

\item $\operatorname*{CD}\left(  a,b\right)  $.

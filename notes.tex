...
%TCIDATA{LastRevised=Monday, February 04, 2019 20:39:00}
...
a_{1}a_{2} &  \equiv b_{1}b_{2}\operatorname{mod}%
n.\label{eq.prop.ent.mod.basics.d.3}%
\end{align}
...
this proof, we WLOG assume that $a\neq0$. Hence, not all of the two numbers
$a,0$ are zero. Thus, $\gcd\left(  a,0\right)  $ is defined to be the largest
...
$\operatorname*{Div}\left(  a\right)  $ are sets of integers). Now, recall
that $\gcd\left(  a,0\right)  $ is the largest element of the set
$\operatorname*{Div}\left(  a,0\right)  $. Thus,
\begin{align*}
\gcd\left(  a,0\right)   &  =\left(  \text{the largest element of the set
}\underbrace{\operatorname*{Div}\left(  a,0\right)  }_{=\operatorname*{Div}%
\left(  a\right)  }\right)  \\
&  =\left(  \text{the largest element of the set }\operatorname*{Div}\left(
a\right)  \right)  =\gcd\left(  a\right)
\end{align*}
(since $\gcd\left(  a\right)  $ is the largest element of the set
$\operatorname*{Div}\left(  a\right)  $).
...
\textbf{(b)} If $a$ and $b$ are both $0$, then Proposition
\ref{prop.ent.gcd.props1} \textbf{(b)} holds for trivial
reasons\footnote{Indeed, if $a$ and $b$ are both $0$, then $\gcd\left(
a,b\right)  =\gcd\left(  b,a\right)  $ (since both $\gcd\left(  a,b\right)  $
and $\gcd\left(  b,a\right)  $ equal $\gcd\left(  0,0\right)  $), and thus
Proposition \ref{prop.ent.gcd.props1} \textbf{(b)} holds.}. Hence, for the
rest of this proof, we WLOG assume that $a$ and $b$ are not both $0$. Hence,
$b$ and $a$ are not both $0$. Thus, $\gcd\left(  b,a\right)  $ is defined to
be the largest element of the set $\operatorname*{Div}\left(  b,a\right)  $
(by Definition \ref{def.ent.gcd.gcd}). Likewise, $\gcd\left(  a,b\right)  $ is
the largest element of the set $\operatorname*{Div}\left(  a,b\right)  $.
...
$\operatorname*{Div}\left(  b,a\right)  $ are sets of integers). Now, recall
that $\gcd\left(  a,b\right)  $ is the largest element of the set
$\operatorname*{Div}\left(  a,b\right)  $. Thus,
\begin{align*}
\gcd\left(  a,b\right)   &  =\left(  \text{the largest element of the set
}\underbrace{\operatorname*{Div}\left(  a,b\right)  }_{=\operatorname*{Div}%
\left(  b,a\right)  }\right)  \\
&  =\left(  \text{the largest element of the set }\operatorname*{Div}\left(
b,a\right)  \right)  =\gcd\left(  b,a\right)
\end{align*}
(since $\gcd\left(  a,b\right)  $ is the largest element of the set
$\operatorname*{Div}\left(  b,a\right)  $). This proves Proposition
\ref{prop.ent.gcd.props1} \textbf{(b)}.

\textbf{(d)} Let $a,b,c\in\mathbb{Z}$ satisfy $b\equiv c\operatorname{mod}a$.
We must prove that $\gcd\left(  a,b\right)  =\gcd\left(  a,c\right)  $.
...
Indeed, for any $b\in\mathbb{Z}$ and any positive integer $a$, we have%
\[
b\%a\equiv b\operatorname{mod}a
\]
(by Corollary \ref{cor.ent.quo-rem.remmod} \textbf{(a)}), thus $b\equiv
b\%a\operatorname{mod}a$ and therefore%
\begin{equation}
\gcd\left(  a,b\right)  =\gcd\left(  a,b\%a\right)
\label{eq.prop.ent.gcd.props1.cor1}%
\end{equation}
(by Proposition \ref{prop.ent.gcd.props1} \textbf{(d)}, applied to $c=b\%a$).

TODO
\end{proof}

\begin{remark}
Proposition \ref{prop.ent.gcd.props1} says that if we add a multiple of $a$ to
$b$, then $\gcd\left(  a,b\right)  $ does not change. Similarly, if we add a
multiple of $b$ to $a$, then $\gcd\left(  a,b\right)  $ does not change (i.e.,
we have $\gcd\left(  vb+a,b\right)  =\gcd\left(  a,b\right)  $ for all
$a,b,v\in\mathbb{Z}$). However, if we \textbf{simultaneously} add a multiple
of $a$ to $b$ and a multiple of $b$ to $a$, then $\gcd\left(  a,b\right)  $
may well change: i.e., we may have $\gcd\left(  vb+a,ua+b\right)  \neq
\gcd\left(  a,b\right)  $ for all $a,b,u,v\in\mathbb{Z}$. Examples are easy to
find (just take $v=1$ and $u=1$).
\end{remark}
...
Let us first consider Case 1. In this case, we have $b\geq0$.
...
\begin{proof}
[Proof of Theorem \ref{thm.ent.gcd.bezout}.]TODO
\end{proof}

An important corollary of Theorem \ref{thm.ent.gcd.bezout} is the following fact:

\begin{theorem}
\label{thm.ent.gcd.uniprop}Let $m,a,b\in\mathbb{Z}$. Then, we have the
...
a,b\right)  \right)  . \label{eq.thm.ent.gcd.uniprop.Div=Div}%
\end{equation}
...
\begin{proof}
[Proof of Theorem \ref{thm.ent.gcd.uniprop}.]Let us first prove
(\ref{eq.thm.ent.gcd.uniprop.equiv}). In order to prove
(\ref{eq.thm.ent.gcd.uniprop.equiv}), we need to prove the \textquotedblleft%
$\Longrightarrow$\textquotedblright\ and \textquotedblleft$\Longleftarrow
$\textquotedblright\ directions of the equivalence
(\ref{eq.thm.ent.gcd.uniprop.equiv}). But this is easy: The \textquotedblleft%
$\Longrightarrow$\textquotedblright\ direction is just the statement of Lemma
\ref{lem.ent.gcd.uniprop}, whereas the \textquotedblleft$\Longleftarrow
$\textquotedblright\ direction is trivial (to wit: if $m\mid\gcd\left(
a,b\right)  $, then
\[
...
equivalence (\ref{eq.thm.ent.gcd.uniprop.equiv}) is proven.{}

The common divisors of $a$ and $b$ are precisely the integers $m$ that satisfy
$\left(  m\mid a\text{ and }m\mid b\right)  $ (by the definition of
\textquotedblleft common divisor\textquotedblright). In view of the
...
and $b$ are precisely the divisors of $\gcd\left(  a,b\right)  $. In other
words,%
\[
\left\{  \text{the common divisors of }a\text{ and }b\right\}  =\left\{
...
\begin{proof}
[Proof of Corollary \ref{cor.ent.gcd.sa,sb}.]TODO.
\end{proof}
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.gcd.div}.]TODO.
\end{proof}
...
\begin{itemize}
\item corollary: existence of modular inverses.
...
\item $\gcd$ of multiple numbers.

\item application: if $k$ is odd then $1+2+\cdots+n\mid1^{k}+2^{k}%
+\cdots+n^{k}$. (illustrates several things.)
...
\bibitem[floor]{Grinbe16}Darij Grinberg, \textit{18.781 (Spring 2016): Floor

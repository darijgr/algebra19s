...
%TCIDATA{LastRevised=Friday, February 08, 2019 18:33:59}
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.gcd.sa,sb,sc}.]TODO! (HW1)
\end{proof}
...
version\textquotedblright\ of common divisors. In this picture, lowest common
multiples corresponding to greatest common divisors.

\begin{definition}
Let $b_{1},b_{2},\ldots,b_{k}$ be integers. Then, the \textit{common
multiples} of $b_{1},b_{2},\ldots,b_{k}$ are defined to be the integers $a$
that satisfy%
\[
...
The \textit{lowest common multiple} (also known as the \textit{least common
...
\begin{proof}
[Proof of Theorem \ref{thm.ent.crt1}.]\textbf{(a)} Theorem
\ref{thm.ent.coprime.modinv} \textbf{(b)} (applied to $m$ instead of $a$)
shows that there exists a $m^{\prime}\in\mathbb{Z}$ such that $mm^{\prime
}\equiv1\operatorname{mod}n$.
...
6\right)  .
\]
To compute such an $x$, follow the proof of Theorem \ref{thm.ent.crt1}
\textbf{(a)} above.
...
\begin{itemize}
\item Note: converse holds too (for $p>1$).
...
\item \textquotedblleft prime factors\textquotedblright\ and canonical factorization

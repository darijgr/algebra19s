...
%TCIDATA{LastRevised=Thursday, February 07, 2019 11:45:54}
...
TODO: Make the following argument readable.

\textit{Induction step:} We are sloppy and induct from $k-1$ to $k$ (the
non-sloppy option would be to induct from $k=\ell-1$ to $\ell$). In other
words, we assume (as induction hypothesis) that the theorem is already proven
\textquotedblleft for $k-1$ instead of $k$\textquotedblright.

Now, let's prove it for $k$.

\textbf{(d)} Let us WLOG assume that $b_{1},b_{2},\ldots,b_{k}$ are not all
$0$ (otherwise, we just want to prove $0=\gcd\left(  0,0\right)  $). Thus,
$\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  >0$. Moreover, at least one of
$\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  $ and $b_{k}$ is nonzero;
hence, $\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
,b_{k}\right)  >0$.

Let $g=\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $ and $h=\gcd\left(
\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  $. It suffices
to prove $g\leq h$ and $h\leq g$.

\textit{Proof of }$g\leq h$\textit{:} It suffices to prove $g\mid h$, since
$g$ and $h$ are positive.

It suffices to prove that $g\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
$ and $g\mid b_{k}$ (because Theorem \ref{thm.ent.gcd.uniprop} \textbf{(a)}
will then yield $g\mid\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots
,b_{k-1}\right)  ,b_{k}\right)  =h$).
...
Thus, $h$ is $\leq$ than the gcd of $b_{1},b_{2},\ldots,b_{k}$ (because the
gcd is literally the greatest common divisor). In other words, $h\leq g$.

Combining $h\leq g$ with $g\leq h$, we obtain $g=h$. In other words,
$\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  =\gcd\left(  \gcd\left(
b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  $. Hence, we are done with
part \textbf{(d)}.
...
,b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  \right)
\ \ \ \ \ \ \ \ \ \ \left(  \text{by Theorem \ref{thm.ent.gcd.uniprop}
\textbf{(a)}}\right) \\
&  \Longleftrightarrow\ \left(  m\mid\gcd\left(  b_{1},b_{2},\ldots
...
\textit{Induction base:} The $k=0$ case is trivial.

\textit{Induction step:} We are sloppy and induct from $k-1$ to $k$ (the
non-sloppy option would be to induct from $k=\ell-1$ to $k=\ell$). In other
words, we assume (as induction hypothesis) that the theorem is already proven
\textquotedblleft for $k-1$ instead of $k$\textquotedblright.

Now, let's prove it for $k$.

By the induction hypothesis, we can apply Theorem \ref{thm.ent.gcd.bezout-mul}
to $k-1$ instead of $k$. We see that there exist $k-1$ integers $y_{1}%
,y_{2},\ldots,y_{k-1}$ such that%
\[
\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  =y_{1}b_{1}+y_{2}b_{2}%
+\cdots+y_{k-1}b_{k-1}.
\]
Consider these $y_{1},y_{2},\ldots,y_{k-1}$.
...
Now, Theorem \ref{thm.ent.gcd.uniprop-mul} \textbf{(d)} yields%
\begin{align*}
\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)   &  =\gcd\left(  \gcd\left(
b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right) \\
...
\end{align*}
This is a linear combination of the $b_{1},b_{2},\ldots,b_{k}$. So Theorem
\ref{thm.ent.gcd.bezout-mul} is proven.
...
& =2\cdot\underbrace{\left(  \left(  1^{d}+2^{d}+\cdots+n^{d}\right)  -\left(
1^{d}+2^{d}+\cdots+\left(  n-1\right)  ^{d}\right)  \right)  }_{=n^{d}}\\

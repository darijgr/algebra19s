...
%TCIDATA{LastRevised=Wednesday, February 13, 2019 12:04:14}
...
\[
\gcd\left(  n,m\right)  =\prod\limits_{p\text{ prime}}p^{\min\left\{
...
\begin{proof}
The infinite products are well-defined.

Let $h=\gcd\left(  n,m\right)  $ and $g=\prod_{p\text{ prime}}p^{\min\left\{
v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}  }$. Want to prove
$g=h$.

We have $n=\prod\limits_{p\text{ prime}}p^{v_{p}\left(  n\right)  }$ (by the
canonical factorization). Hence, $g\mid n$, since each prime $p$ satisfies
$p^{\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(  m\right)  \right\}
}\mid p^{v_{p}\left(  n\right)  }$ (since $\min\left\{  v_{p}\left(  n\right)
,v_{p}\left(  m\right)  \right\}  \leq v_{p}\left(  n\right)  $). Similarly,
$g\mid m$. Combining these, we obtain $g\mid\gcd\left(  n,m\right)  =h$.

On the other hand, fix any prime $p$. We have $h=\gcd\left(  n,m\right)  \mid
n$, thus $v_{p}\left(  h\right)  \leq v_{p}\left(  n\right)  $ (by Proposition
above). Similarly, $v_{p}\left(  h\right)  \leq v_{p}\left(  m\right)  $.
...
,v_{p}\left(  m\right)  \right\}  }$. Multiplying this over all primes $p$, we
find%
\[
\prod\limits_{p\text{ prime}}p^{v_{p}\left(  h\right)  }\mid\prod
\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(  n\right)  ,v_{p}\left(
m\right)  \right\}  }=g.
\]
But $h>0$, so $h=\prod\limits_{p\text{ prime}}p^{v_{p}\left(  h\right)  }$ (by
the canonical factorization). Hence, $h=\prod\limits_{p\text{ prime}}%
p^{v_{p}\left(  h\right)  }\mid g$. Combined with $g\mid h$, this yields
$\left\vert g\right\vert =\left\vert h\right\vert $, which rewrites as $g=h$.

Thus, the equality for $\gcd\left(  n,m\right)  $ is proven.

The proof of the equality for $\operatorname{lcm}\left(  n,m\right)  $ is
analogous -- you need to flip every divisibility and equality and replace
$\min$ by $\max$.
\end{proof}
...
\cdot\underbrace{\prod_{\text{other primes }p}p^{\max\left\{  0,0\right\}  }%
}_{=1}\\
...
\begin{remark}
We can use the proposition above to re-prove various properties of $\gcd$ and
$\operatorname{lcm}$. For instance, let us prove that
\[
\gcd\left(  a,b\right)  \cdot\operatorname{lcm}\left(  a,b\right)  =\left\vert
ab\right\vert
\]
for any integers $a,b$. Indeed, WLOG assume that $a$ and $b$ are nonzero.
Then, the canonical factorization of $\left\vert a\right\vert $ is%
\[
...
\min\left\{  u,v\right\}  +\max\left\{  u,v\right\}  =u+v\text{ for all
}u\text{ and }v\text{)}}}\\
...
}\right)  }_{=\left\vert a\right\vert }\underbrace{\left(  \prod_{p\text{
prime}}p^{v_{p}\left(  b\right)  }\right)  }_{=\left\vert b\right\vert
}=\left\vert a\right\vert \cdot\left\vert b\right\vert =\left\vert
ab\right\vert .
\end{align*}
...
bcd,cda,dab,abc\right)    & =\left\vert abcd\right\vert .
\end{align*}
For example, let us see why the first of these holds:%
\begin{align*}
...
& =\left(  \prod\limits_{p\text{ prime}}p^{\min\left\{  v_{p}\left(  a\right)
...
\begin{proposition}
Let $n$ and $m$ be two integers. Then, $n\perp m$ if and only if no prime
divides both $n$ and $m$.
\end{proposition}

\begin{proof}
$\Longrightarrow:$ Assume that $n\perp m$. Thus, $\gcd\left(  n,m\right)  =1$.
If a prime $p$ would divide both $n$ and $m$, then it would divide
$\gcd\left(  n,m\right)  =1$, which is impossible (since $1$ is not a prime).
So no prime divides both $n$ and $m$.

$\Longleftarrow:$ Assume that no prime divides both $n$ and $m$. If we had
$\gcd\left(  n,m\right)  >1$, then $\gcd\left(  n,m\right)  $ would have a
prime divisor $p$, and then this prime $p$ would divide both $n$ and $m$; but
this would contradict the assumption. Hence, $\gcd\left(  n,m\right)  \leq1$.
Thus, $\gcd\left(  n,m\right)  =1$. Thus, $n\perp m$.
\end{proof}

\begin{corollary}
Let $n$ and $m$ be nonzero integers. Then, $n\perp m$ if and only if%
\[
\sum_{p\text{ primes}}v_{p}\left(  n\right)  v_{p}\left(  m\right)  =0.
\]
...
We have the equivalence%
\begin{align*}
& \ \left(  \sum_{p\text{ primes}}v_{p}\left(  n\right)  v_{p}\left(
m\right)  =0\right)  \\
& \Longleftrightarrow\ \left(  v_{p}\left(  n\right)  v_{p}\left(  m\right)
=0\text{ for each prime }p\right)  \\
...
}v_{p}\left(  m\right)  =0\right)  \text{ for each prime }p\right)  \\
& \Longleftrightarrow\ \left(  \left(  p\nmid n\text{ or }p\nmid m\right)
\text{ for each prime }p\right)  \\
& \Longleftrightarrow\ \left(  \text{no prime divides both }n\text{ and
}m\right)  \\
& \Longleftrightarrow\ \left(  n\perp m\right)  \ \ \ \ \ \ \ \ \ \ \left(
\text{by previous proposition}\right)  .
\end{align*}
...
We shall find a new prime distinct from $p_{1},p_{2},\ldots,p_{k}$.

Let $n=p_{1}p_{2}\cdots p_{k}+1$. Then, $n>1$. Thus, $n$ has a prime divisor
$p$. Consider this $p$.

\textit{Claim:} $p\neq p_{i}$ for all $i$.

[\textit{Proof of claim:} Assume the contrary. Thus, $p=p_{i}$ for some $i$.
Consider this $i$. We have $n-1=p_{1}p_{2}\cdots p_{k}$, which is divisible by
$p_{i}$. In other words, $n\equiv1\operatorname{mod}p_{i}$. In other words,
$n\equiv1\operatorname{mod}p$ (since $p=p_{i}$).

However, $p\mid n$, so $n\equiv0\operatorname{mod}p$. So $0\equiv
n\equiv1\operatorname{mod}p$, which means $p\mid1-0=1$. This is absurd.]

The claim shows that the prime $p$ is distinct from $p_{1},p_{2},\ldots,p_{k}$.
\end{proof}
...
\begin{definition}
We define a function $\phi:\mathbb{P}\rightarrow\mathbb{N}$ as follows: For
each $n\in\mathbb{P}$, let%
\begin{align*}
\phi\left(  n\right)    & =\left(  \text{the number of all }i\in\left\{
1,2,\ldots,n\right\}  \text{ that are coprime to }n\right)  \\
...
\begin{example}
\textbf{(a)} $\phi\left(  12\right)  =4$, since the $i\in\left\{
1,2,\ldots,12\right\}  $ that are coprime to $12$ are $1,5,7,11$.
\[
...
\textbf{(b)} $\phi\left(  13\right)  =12$, since all $i\in\left\{
1,2,\ldots,12\right\}  $ are coprime to $13$.

\textbf{(c)} $\phi\left(  14\right)  =6$, since%
\[
...
\begin{exercise}
Let $p$ be a prime. Let $k$ be a positive integer. Prove that $\phi\left(
...
\left(  p-1\right)  p^{v_{p}\left(  n\right)  -1}\right)  \\
...
We will show this as a consequence of some basic ring theory later on.
...
Again, we will prove this later.
...
that }\gcd\left(  i,n\right)  =d\right)  \\
& =\phi\left(  n/d\right)  .
\end{align*}
...
n/d\right\}  \right\vert \\
& =\phi\left(  n/d\right)  \ \ \ \ \ \ \ \ \ \ \left(  \text{by the definition
of }\phi\right)  .
\end{align*}

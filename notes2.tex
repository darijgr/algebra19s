...
%TCIDATA{LastRevised=Wednesday, February 06, 2019 12:05:16}
...
\begin{theorem}
Let $b_{1},b_{2},\ldots,b_{k}$ be some integers.

\textbf{(a)} For each $m\in\mathbb{Z}$, we have the following logical
equivalence:%
\[
\left(  m\mid b_{i}\text{ for all }i\in\left\{  1,2,\ldots,k\right\}  \right)
\Longleftrightarrow\left(  m\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)
\right)  .
\]
...
\begin{proof}
We will prove the theorem by induction on $k$.

Base case ($k=0$) is trivial.

Step: We want to prove the theorem for some $k$, assuming (as induction
hypothesis) that it is already proven for $k-1$.

\textbf{(d)} Set $g=\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $

and $h=\gcd\left(  \gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)
,b_{k}\right)  $.

We want to prove $g=h$. Enough to prove $g\leq h$ and $h\leq g$.

WLOG assume not all of $b_{1},b_{2},\ldots,b_{k-1}$ are $0$.

\textit{Proof of }$g\leq h$\textit{:} We have $g=\gcd\left(  b_{1}%
,b_{2},\ldots,b_{k}\right)  \mid b_{i}$ for all $i$.

Thus, in particular, $g\mid b_{i}$ for all $i\in\left\{  1,2,\ldots
,k-1\right\}  $. Thus, part \textbf{(a)} (applied to $k-1$ instead of $k$)
yields that $g\mid\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  $ (by
induction hypothesis). Also, $g\mid b_{k}$. Hence, $g\mid\gcd\left(
\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  $ (by Theorem
2.41 \textbf{(a)}). In other words, $g\mid h$. Thus, $g\leq h$ (since $h>0$).

\textit{Proof of }$h\leq g$\textit{:} We have $h=\gcd\left(  \gcd\left(
b_{1},b_{2},\ldots,b_{k-1}\right)  ,b_{k}\right)  $. We must prove that $h\leq
g=\gcd\left(  b_{1},b_{2},\ldots,b_{k}\right)  $. For that it suffices to show
that $h$ is a common divisor of $b_{1},b_{2},\ldots,b_{k}$. This follows from
the fact that%
\begin{align*}
...
\mid b_{k}.
\]
So $h\leq g$ is proven.

Combining $g\leq h$ with $h\leq g$, we get $g=h$. Part \textbf{(d)} is thus proven.

\textbf{(a)} Let $m\in\mathbb{Z}$. Then, we have the following equivalence:%
\begin{align*}
...
,\ldots,b_{k-1}\right)  \right)  \\\text{(by induction hypothesis)}}}\text{
and }m\mid b_{k}\right)  \\
...
\textbf{(d)}, which we proved above}\right)  .
\end{align*}
Thus part \textbf{(a)} is proven.

\textbf{(b)} is a restatement of \textbf{(a)}.

\textbf{(c)} is a restatement of \textbf{(b)}.

\textbf{(e)} By induction hypothesis, there exist integers $y_{1},y_{2}%
,\ldots,y_{k-1}$ such that%
\[
\gcd\left(  b_{1},b_{2},\ldots,b_{k-1}\right)  =y_{1}b_{1}+y_{2}b_{2}%
+\cdots+y_{k-1}b_{k-1}.
\]
Also, by Bezout's theorem, there are integers $u$ and $v$ such that
\[
...
& =x_{1}b_{1}+x_{2}b_{2}+\cdots+x_{k}b_{k},
\end{align*}
where $x_{i}=%
\begin{cases}
uy_{i}, & \text{if }i<k;\\
...
\begin{definition}
Let $a$ and $b$ be two integers. We say that $a$ is \textbf{coprime} (or
\textbf{relatively prime}) to $b$ if and only if $\gcd\left(  a,b\right)  =1$.

This relation is symmetric (since $\gcd\left(  a,b\right)  =\gcd\left(
b,a\right)  $), so it is also called \textquotedblleft$a$ and $b$ are
\textbf{coprime}\textquotedblright.
...
$\backslash$%
%EndExpansion
perp gives the $\perp$ sign.)
\end{definition}

\begin{example}
\textbf{(a)} The numbers $2$ and $3$ are coprime, since $\gcd\left(
2,3\right)  =1$.

\textbf{(b)} The numbers $6$ and $15$ are not coprime, since $\gcd\left(
6,15\right)  =3$.

\textbf{(c)} Let $a\in\mathbb{Z}$. Then, $a$ is coprime to $a+1$, since%
\begin{align*}
...
& \mid1
\end{align*}
and thus $\gcd\left(  a,a+1\right)  =1$.

\textbf{(d)} When is $a$ coprime to $a+2$ ? We have%
\begin{align*}
...
\end{cases}
\end{align*}
Thus, $a$ is coprime to $a+2$ if and only if $a$ is odd.
\end{example}

\begin{theorem}
Let $a,b,c\in\mathbb{Z}$ satisfy $a\mid bc$ and $a\perp b$. Then, $a\mid c$.
\end{theorem}

\begin{theorem}
Let $a,b,c\in\mathbb{Z}$ satisfy $a\mid c$ and $b\mid c$ and $a\perp b$. Then,
$ab\mid c$.
\end{theorem}
...
$ab\equiv1\operatorname{mod}n$. Such an integer $b$ is called a
\textbf{modular inverse} of $a$ modulo $n$.
\end{theorem}

\begin{proof}
\textbf{(a)} Bezout yields that $\gcd\left(  a,n\right)  =xa+yn$ for some
integers $x,y$. Thus, $ax=xa\equiv xa+yn\operatorname{mod}n$ (since
$xa-\left(  xa+yn\right)  =-yn=n\left(  -y\right)  $ is clearly divisible by
$n$). Hence, take $b=x$ and get $ab=ax\equiv xa+yn=\gcd\left(  a,n\right)
\operatorname{mod}n$.
...
Similar formulas exist for higher powers, by the way:
...
Observe: For any $d\in\mathbb{N}$, we can write $\sum_{k=1}^{n}k^{d}$ as a
polynomial of degree $d+1$ in $n$ with rational coefficients. The values of
this polynomial at integer $n$'s are always integers, but its coefficients are not.

\begin{exercise}
Let $n\in\mathbb{N}$. Let $d$ be an \textbf{odd} positive integer. Then,%
\[
...
\begin{proof}
Recalling that $1+2+\cdots+n=\dfrac{n\left(  n+1\right)  }{2}$, we can rewrite
this as%
\[
...
\[
n\left(  n+1\right)  \mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  .
\]
Since $n$ and $n+1$ are coprime, we only need to check that%
\begin{align*}
n  & \mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
\ \ \ \ \ \ \ \ \ \ \text{and}\\
n+1  & \mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  .
\end{align*}
This is not too hard. Let us prove the second divisibility:

I need a lemma:

\begin{lemma}
Let $x$ and $y$ be two integers, and $d\in\mathbb{N}$.
...
[\textit{Proof of lemma:} \textbf{(a)} follows from HW\#0 exercise 1.

\textbf{(b)} Assume $d$ is odd. Then, we can apply part \textbf{(a)} to $-y$
instead of $y$ and get $x-\left(  -y\right)  \mid x^{d}-\left(  -y\right)
^{d}$. Since $d$ is odd, this rewrites as $x+y\mid x^{d}+y^{d}$.]

\textit{Proof of }$n+1\mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
$\textit{:} We have%
\begin{align*}
& 2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  \\
& =\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  +\left(  1^{d}+2^{d}+\cdots
+n^{d}\right)  \\
& =\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  +\left(  n^{d}+\left(  n-1\right)
^{d}+\cdots+1^{d}\right)  \\
& =\sum_{k=1}^{n}k^{d}+\sum_{k=1}^{n}\left(  n+1-k\right)  ^{d}=\sum_{k=1}%
^{n}\underbrace{\left(  k^{d}+\left(  n+1-k\right)  ^{d}\right)
}_{\substack{\text{This is divisible by }k+\left(  n+1-k\right)  \\\text{(by
part \textbf{(b)} of the Lemma),}\\\text{which means it is divisible by }%
n+1}}\\
& =\sum_{k=1}^{n}\left(  \text{things divisible by }n+1\right)  ,
\end{align*}
and this is clearly divisible by $n+1$.

\textit{Proof of }$n\mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
$\textit{:} Apply
\[
n+1\mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)
\]
to $n-1$ instead of $n$. This yields%
\[
n\mid2\cdot\left(  1^{d}+2^{d}+\cdots+\left(  n-1\right)  ^{d}\right)  .
\]
How does this yield $n\mid2\cdot\left(  1^{d}+2^{d}+\cdots+n^{d}\right)  $ ?
\end{proof}

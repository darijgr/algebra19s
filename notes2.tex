...
%TCIDATA{LastRevised=Friday, February 08, 2019 12:05:31}
...
\end{align*}
This proves Theorem \ref{thm.ent.gcd.uniprop-mul} \textbf{(a)}. Parts
\textbf{(b)} and \textbf{(c)} of Theorem \ref{thm.ent.gcd.uniprop-mul} are
restatements of Theorem \ref{thm.ent.gcd.uniprop-mul} \textbf{(a)} and can be
derived from it in the same way as we derived parts \textbf{(b)} and
\textbf{(c)} of Theorem \ref{thm.ent.gcd.uniprop} from Theorem
\ref{thm.ent.gcd.uniprop} \textbf{(a)}.
...
\subsection{More properties of coprimality}

\begin{theorem}
Let $a,b,c\in\mathbb{Z}$ such that $a\perp c$ and $b\perp c$. Then, $ab\perp
c$.
\end{theorem}

\begin{proof}
We have $\gcd\left(  a,c\right)  =1$ (since $a\perp c$). Bezout says that
there are $u,v\in\mathbb{Z}$ such that $ua+vc=\gcd\left(  a,c\right)  =1$.

Similarly, Bezout says that there are $x,y\in\mathbb{Z}$ such that
$xb+yc=\gcd\left(  b,c\right)  =1$.

Multiplying these two equalities, we obtain $\left(  ua+vc\right)  \left(
xb+yc\right)  =1\cdot1=1$. Thus,%
\begin{align*}
1  & =\left(  ua+vc\right)  \left(  xb+yc\right)  \\
& =uxab+uyac+vxbc+vycc\\
& =\left(  ux\right)  \left(  ab\right)  +\left(  uya+vxb+vyc\right)  c.
\end{align*}
So $1$ is a $\mathbb{Z}$-linear combination of $ab$ and $c$. Now, $\gcd\left(
ab,c\right)  $ divides any such combination. Thus, $\gcd\left(  ab,c\right)
\mid1$. Since gcds are positive, this yields $\gcd\left(  ab,c\right)  =1$, so
that $ab\perp c$.
\end{proof}

\begin{exercise}
Let $c\in\mathbb{Z}$. Let $a_{1},a_{2},\ldots,a_{k}$ be integers such that
each $i$ satisfies $a_{i}\perp c$. Prove that $a_{1}a_{2}\cdots a_{k}\perp c$.
\end{exercise}

\begin{proof}
Induction on $k$.

\textit{Base:} $k=0$, then $\left(  \text{empty product}\right)  =1\perp c$
since $\gcd\left(  1,c\right)  =1$.

\textit{Step:} Assume the exercise is true for $k-1$. We must prove it for $k$.

By induction hypothesis, we have $a_{1}a_{2}\cdots a_{k-1}\perp c$. But by
assumption, $a_{k}\perp c$. Thus, the previous theorem yields $\left(
a_{1}a_{2}\cdots a_{k-1}\right)  a_{k}\perp c$. In other words, $a_{1}%
a_{2}\cdots a_{k}\perp c$. So the step is complete, and the exercise is solved.
\end{proof}

We can generalize Theorem 2.59 to several divisors:

\begin{exercise}
Let $c\in\mathbb{Z}$. Let $b_{1},b_{2},\ldots,b_{k}$ be divisors of $c$ that
are mutually coprime (i.e., satisfy $b_{i}\perp b_{j}$ for all $i\neq j$).
Then, $b_{1}b_{2}\cdots b_{k}\mid c$.
\end{exercise}

\begin{proof}
Induction on $k$.

\textit{Base:} $k=0$, then $\left(  \text{empty product}\right)  =1\mid c$.

\textit{Step:} Assume the exercise is true for $k-1$. We must prove it for $k$.

By induction hypothesis, we have $b_{1}b_{2}\cdots b_{k-1}\mid c$. But by
assumption, $b_{k}\mid c$. Thus, Theorem 2.59 (applied to $a=$ $b_{1}%
b_{2}\cdots b_{k-1}$ and $b=b_{k}$) would yield $\left(  b_{1}b_{2}\cdots
b_{k-1}\right)  b_{k}\mid c$, if we can show that $b_{1}b_{2}\cdots
b_{k-1}\perp b_{k}$. But the previous exercise does yield $b_{1}b_{2}\cdots
b_{k-1}\perp b_{k}$, so we can make this conclusion and get $\left(
b_{1}b_{2}\cdots b_{k-1}\right)  b_{k}\mid c$. In other words, $b_{1}%
b_{2}\cdots b_{k}\mid c$. So the step is complete, and the exercise is solved.
\end{proof}

\begin{exercise}
Let $a,b\in\mathbb{Z}$ be such that $a\perp b$. Let $n,m\in\mathbb{N}$. Then,
$a^{n}\perp b^{m}$.
\end{exercise}

\begin{proof}
We have $a\perp b$, thus $\underbrace{aa\cdots a}_{n\text{ times}}\perp b$ (by
the second-to-last exercise). In other words, $a^{n}\perp b$. In other words,
$b\perp a^{n}$. Thus, $\underbrace{bb\cdots b}_{m\text{ times}}\perp a^{n}$
(by the second-to-last exercise). In other words, $b^{m}\perp a^{n}$, so that
$a^{n}\perp b^{m}$.
\end{proof}
...
\begin{definition}
Let $b_{1},b_{2},\ldots,b_{k}$ be integers.

\textbf{(a)} A \textbf{common multiple} of $b_{1},b_{2},\ldots,b_{k}$ is an
integer $a$ such that $\left(  b_{i}\mid a\text{ for each }i\right)  $. In
other words, it is an integer that is a multiple of each of $b_{1}%
,b_{2},\ldots,b_{k}$.

\textbf{(b)} The \textbf{lowest common multiple} (aka \textbf{least common
multiple}) of $b_{1},b_{2},\ldots,b_{k}$ is

\begin{itemize}
\item the smallest positive common multiple of $b_{1},b_{2},\ldots,b_{k}$, if
none of $b_{1},b_{2},\ldots,b_{k}$ are $0$;

\item $0$, if some of $b_{1},b_{2},\ldots,b_{k}$ are $0$.
\end{itemize}

This is well-defined, because if none of $b_{1},b_{2},\ldots,b_{k}$ are $0$,
then $\left\vert b_{1}b_{2}\cdots b_{k}\right\vert $ is a positive common
multiple of $b_{1},b_{2},\ldots,b_{k}$.
\end{definition}

\begin{theorem}
Let $a,b\in\mathbb{Z}$. Then, $\gcd\left(  a,b\right)  \cdot\operatorname{lcm}%
\left(  a,b\right)  =\left\vert ab\right\vert $.
\end{theorem}

\begin{proof}
WLOG assume that none of $a,b$ is $0$. Let $c=\dfrac{ab}{\gcd\left(
a,b\right)  }$. Then,%
\[
c=a\cdot\underbrace{\dfrac{b}{\gcd\left(  a,b\right)  }}_{\in\mathbb{Z}}.
\]
Thus, $c$ is an integer and a multiple of $a$. Similarly, $c$ is a multiple of
$b$. Thus, $c$ is a common multiple of $a$ and $b$. Hence, $\left\vert
c\right\vert $ is a positive common multiple of $a$ and $b$. Therefore,
$\left\vert c\right\vert \geq\operatorname{lcm}\left(  a,b\right)  $ (since
$\operatorname{lcm}\left(  a,b\right)  $ is the smallest such multiple).

Next, let me prove that $\left\vert c\right\vert \leq\operatorname{lcm}\left(
a,b\right)  $.

Indeed, let $\ell=\operatorname{lcm}\left(  a,b\right)  $, and let me prove
that $c\mid\ell$.

Theorem 2.43 (applied to $\ell$ instead of $c$) yields $ab\mid\gcd\left(
a,b\right)  \cdot\ell$ (since $a\mid\ell$ and $b\mid\ell$). Hence, $\dfrac
{ab}{\gcd\left(  a,b\right)  }\mid\ell$. In other words, $c\mid\ell$. Thus,
$\left\vert c\right\vert \leq\ell$ (since $\ell>0$). In other words,
$\left\vert c\right\vert \leq\operatorname{lcm}\left(  a,b\right)  $. Combined
with $\left\vert c\right\vert \geq\operatorname{lcm}\left(  a,b\right)  $,
this becomes $\left\vert c\right\vert =\operatorname{lcm}\left(  a,b\right)
$. Hence,%
\[
\operatorname{lcm}\left(  a,b\right)  =\left\vert c\right\vert =\left\vert
\dfrac{ab}{\gcd\left(  a,b\right)  }\right\vert =\dfrac{\left\vert
ab\right\vert }{\gcd\left(  a,b\right)  }.
\]
In other words, $\gcd\left(  a,b\right)  \cdot\operatorname{lcm}\left(
a,b\right)  =\left\vert ab\right\vert $.
\end{proof}
...
=\operatorname*{Mul}\left(  \gcd\left(  a,b\right)  \right)  $, where
$\operatorname*{Mul}$ means \textquotedblleft set of multiples
of\textquotedblright.
\end{theorem}
...
\begin{proposition}
For $k>0$ integers $b_{1},b_{2},\ldots,b_{k}$, we have%
\[
...
\subsection{The Chinese Remainder Theorem (baby form)}

\begin{theorem}
Let $m$ and $n$ be two coprime integers. Let $a,b\in\mathbb{Z}$.
...
\textbf{(b)} This integer $x$ is \textquotedblleft unique modulo
$mn$\textquotedblright. That is, if $x_{1}$ and $x_{2}$ are two such integers
$x$, then $x_{1}\equiv x_{2}\operatorname{mod}mn$.
\end{theorem}

This is known as the \textit{Chinese Remainder Theorem} (Sunzi, 3rd Century AD).
...
6\right)  .
\]
The theorem says that we can, since $5$ and $6$ are coprime.
\end{example}

\begin{proof}
\textbf{(a)} Theorem 2.60 \textbf{(b)} yields that there exists an $m^{\prime
}\in\mathbb{Z}$ such that $mm^{\prime}\equiv1\operatorname{mod}n$ (because
$m\perp n$).

Similarly, there exists an $n^{\prime}\in\mathbb{Z}$ such that $nn^{\prime
}\equiv1\operatorname{mod}m$.

Now, set $x=nn^{\prime}a+mm^{\prime}b$. Then,%
\[
x=nn^{\prime}a+\underbrace{mm^{\prime}b}_{\equiv0\operatorname{mod}m}%
\equiv\underbrace{nn^{\prime}}_{\equiv1\operatorname{mod}m}a\equiv
a\operatorname{mod}m.
\]
Similarly, $x\equiv b\operatorname{mod}n$. Thus we have found our $x$.

\textbf{(b)} Let $x_{1}$ and $x_{2}$ be two such integers $x$. We must prove
that $x_{1}\equiv x_{2}\operatorname{mod}mn$.

We have $x_{1}\equiv a\operatorname{mod}m$ and $x_{2}\equiv
a\operatorname{mod}m$, so that $x_{1}\equiv a\equiv x_{2}\operatorname{mod}m$.
In other words, $m\mid x_{1}-x_{2}$. Similarly, $n\mid x_{1}-x_{2}$. Since $m$
and $n$ are coprime, these yields $mn\mid x_{1}-x_{2}$ (by Theorem 2.59). In
other words, $x_{1}\equiv x_{2}\operatorname{mod}mn$.
\end{proof}
...
6\right)  .
\]
To compute such an $x$, follow the proof of Theorem \textbf{(a)} above.

We need a modular inverse $5^{\prime}$ of $5$ modulo $6$. Such an inverse is
$5$, since $5\cdot5\equiv1\operatorname{mod}6$. (You can find modular inverses
...
Now, the proof of the Theorem says $x=6\cdot6^{\prime}\cdot3+5\cdot5^{\prime
}\cdot2=6\cdot1\cdot3+5\cdot5\cdot2=\allowbreak68$.

Checking this: $68\equiv3\operatorname{mod}5$ since $68-3=\allowbreak5\cdot
13$; and $68\equiv2\operatorname{mod}6$ since $68-2=6\cdot11$.
\end{example}

There is also a version of the Chinese Remainder Theorem for multiple integers:

\begin{theorem}
Let $m_{1},m_{2},\ldots,m_{k}$ be $k$ mutually coprime integers. Let
$a_{1},a_{2},\ldots,a_{k}\in\mathbb{Z}$.

\textbf{(a)} There exists an integer $x$ such that%
\[
\left(  x\equiv a_{i}\operatorname{mod}m_{i}\text{ for all }i\right)  .
\]


\textbf{(b)} This integer $x$ is \textquotedblleft unique modulo $m_{1}%
m_{2}\cdots m_{k}$\textquotedblright: That is, if $x_{1}$ and $x_{2}$ are two
such integers $x$, then $x_{1}\equiv x_{2}\operatorname{mod}m_{1}m_{2}\cdots
m_{k}$.
\end{theorem}
...
\begin{definition}
Let $p$ be an integer greater than $1$. We say that $p$ is \textbf{prime} if
the only divisors of $p$ are $1$ and $p$. A prime integer is often just called
\textbf{a prime}.
\end{definition}

Thus, primes have to be $>1$. So $1$ does not count.

How do we quickly find all primes $\leq30$?%
\begin{align*}
& \text{\textit{1}},2,3,\text{4},5,\text{6},7,\text{8},\text{9},\text{10},\\
& 11,\text{12},13,\text{14},\text{15},\text{16},17,\text{18},19,\text{20},\\
& \text{21},\text{22},23,\text{24},\text{25},\text{26},\text{27}%
,\text{28},29,\text{30}%
\end{align*}
The red numbers are the primes. This is called the \textbf{sieve of
Eratosthenes}.

\begin{proposition}
Let $p$ be a prime. Then, each $i\in\left\{  1,2,\ldots,p-1\right\}  $ is
coprime to $p$.
\end{proposition}

\begin{proof}
Let $i\in\left\{  1,2,\ldots,p-1\right\}  $. Then, $\gcd\left(  i,p\right)  $
is a positive divisor of $p$, thus must be $1$ or $p$ (since $p$ is prime).
But it cannot be $p$, since it has to be $\leq i<p$. So it must be $1$. But
this means $i\perp p$.
\end{proof}

\begin{theorem}
Let $p$ be a prime. Let $a,b\in\mathbb{Z}$ such that $p\mid ab$. Then, $p\mid
a$ or $p\mid b$.
\end{theorem}

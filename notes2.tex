...
%TCIDATA{LastRevised=Monday, February 11, 2019 12:04:46}
...
But $\gcd\left(  i,p\right)  \mid i$, thus $\gcd\left(  i,p\right)  \leq i\leq
p-1<p$, so $\gcd\left(  i,p\right)  \neq p$.

But $\gcd\left(  i,p\right)  \mid p$, thus $\gcd\left(  i,p\right)  =1$ or
$\gcd\left(  i,p\right)  =p$ (since the only divisors of $p$ are $1$ and $p$).
In view of $\gcd\left(  i,p\right)  \neq p$, we thus get $\gcd\left(
i,p\right)  =1$.
\end{proof}

\begin{proposition}
Let $p$ be a prime. Let $a\in\mathbb{Z}$. Then, either $p\mid a$ or $p\perp a$.
\end{proposition}

\begin{proof}
Assume the contrary. Thus, neither $p\mid a$ nor $p\perp a$.

Recall: $p\mid a$ if and only if $a\%p=0$. Thus, $a\%p\neq0$.

But $a\%p\in\left\{  0,1,\ldots,p-1\right\}  $. Hence, $a\%p\in\left\{
0,1,\ldots,p-1\right\}  \setminus\left\{  0\right\}  =\left\{  1,2,\ldots
,p-1\right\}  $. So we can apply the previous proposition to $i=a\%p$. We thus
conclude that $a\%p$ is coprime to $p$. In other words, $\gcd\left(
p,a\%p\right)  =1$.

Proposition 2.9.7 \textbf{(e)} yields $\gcd\left(  p,a\right)  =\gcd\left(
p,a\%p\right)  =1$, so $p\perp a$. Contradiction to assumption!
\end{proof}
...
\begin{proof}
Assume the contrary. Thus, neither $p\mid a$ nor $p\mid b$.

The previous proposition yields either $p\mid a$ or $p\perp a$. Hence, $p\perp
a$. 

But $p\mid ab$. Hence, Theorem 2.10.6 (applied to $p$, $a$ and $b$ instead of
$a$, $b$ and $c$) yields $p\mid b$. Contradiction!
\end{proof}
...
\begin{proof}
Let $d$ be the \textbf{smallest} divisor of $n$ that is $>1$.

We claim that $d$ is a prime. Indeed, any divisor of $d$ other than $1$ and
$d$ would be an even smaller divisor of $n$ that is $>1$; but that is
impossible. So the only divisors of $d$ are $1$ and $d$. In other words, $d$
is prime. So take $p=d$.
\end{proof}

\begin{definition}
Let $n$ be an integer. A \textbf{prime factor} (or \textbf{prime divisor}) of
$n$ means a prime $p$ such that $p\mid n$.
\end{definition}
...
\begin{exam}
\textbf{(a)} $60=2\cdot30=2\cdot2\cdot15=2\cdot2\cdot3\cdot5$. The right hand
side is a product of $4$ primes.

\textbf{(b)} $7=7$. This is a product of $1$ prime.

\textbf{(c)} $1=\left(  \text{empty product}\right)  $. This is a product of
$0$ primes.
\end{exam}

\begin{proof}
The proposition follows from the previous one by strong induction.
\end{proof}

\begin{definition}
Let $n$ be a positive integer.

A \textbf{prime factorization} of $n$ means a way to write $n$ as a product of
finitely many primes.

Rigorously:

A \textbf{prime factorization} of $n$ means a tuple $\left(  p_{1}%
,p_{2},\ldots,p_{k}\right)  $ of primes such that $n=p_{1}p_{2}\cdots p_{k}$.
\end{definition}
...
,p}_{i\text{ times}}\right)  $. This is not quite trivial.

\textbf{(d)} The only prime factorization of $1$ is the $0$-tuple $\left(
{}\right)  $.
\end{exam}

These examples suggest that the prime factorization of an $n$ is unique up to
reordering the entries. This is true, and we will see it soon.
...
denote the largest $m\in\mathbb{N}$ such that $p^{m}\mid n$. This is
well-defined, since $p^{m}\mid n$ implies $p^{m}\leq\left\vert n\right\vert $.

\textbf{(b)} Set $v_{p}\left(  0\right)  =\infty$, where $\infty$ is a symbol
which we take to satisfy%
\begin{align*}
\infty+k  & =\infty;\\
k  & <\infty\ \ \ \ \ \ \ \ \ \ \text{for all integers }k;\\
\min\left(  S\cup\left\{  \infty\right\}  \right)    & =\min
S\ \ \ \ \ \ \ \ \ \ \text{if }S\neq\varnothing;\\
...
\begin{lemma}
Let $p$ be a prime. Let $i\in\mathbb{N}$ and $n\in\mathbb{Z}$. Then,
...
$m$ (by definition). So $v_{p}\left(  n\right)  \geq i$.

$\Longleftarrow:$ Assume $v_{p}\left(  n\right)  \geq i$. Recall that
$v_{p}\left(  n\right)  $ is the largest $m\in\mathbb{N}$ such that $p^{m}\mid
n$. In particular, $p^{v_{p}\left(  n\right)  }\mid n$. But $v_{p}\left(
n\right)  \geq i$, so that $i\leq v_{p}\left(  n\right)  $ and thus $p^{i}\mid
p^{v_{p}\left(  n\right)  }\mid n$.
\end{proof}

\begin{theorem}
Let $p$ be a prime. Let $a,b\in\mathbb{Z}$.
...
\begin{proof}
\textbf{(a)} WLOG assume that $a,b\neq0$.

The definition of $v_{p}\left(  a\right)  $ yields that $p^{v_{p}\left(
a\right)  }\mid a$ but $p^{v_{p}\left(  a\right)  +1}\nmid a$.

(The symbol $\nmid$ means \textquotedblleft does not divide\textquotedblright.

The symbol $\not \equiv $ means \textquotedblleft is not congruent
to\textquotedblright.)

Thus, $x:=a/p^{v_{p}\left(  a\right)  }$ is an integer but is not divisible by
$p$ (since $p^{v_{p}\left(  a\right)  +1}\nmid a$). Hence, $x\perp p$ (by one
of our previous propositions).

Similarly, $y:=b/p^{v_{p}\left(  b\right)  }$ is also an integer and satisfies
$y\perp p$.
...
\]
is an integer and is coprime to $p$ (since $x\perp p$ and $y\perp p$). Hence,
it is not divisible by $p$. In other words, $p^{v_{p}\left(  a\right)
+v_{p}\left(  b\right)  +1}\nmid ab$. But $p^{v_{p}\left(  a\right)
+v_{p}\left(  b\right)  }\mid ab$ (since $ab/p^{v_{p}\left(  a\right)
+v_{p}\left(  b\right)  }$ is an integer). Hence, $v_{p}\left(  ab\right)
...
\begin{corollary}
Let $p$ be a prime. Let $a_{1},a_{2},\ldots,a_{k}$ be integers. Then,
\[
...
be a prime factorization of $n$. Let $p$ be a prime. Then,
\begin{align*}
...
(because it appears $v_{p}\left(  n\right)  $ many times in each of these two
factorizations, by the Proposition above). Hence, the factorizations $\left(
a_{1},a_{2},\ldots,a_{k}\right)  $ and $\left(  b_{1},b_{2},\ldots,b_{\ell
}\right)  $ have the same entries appear equally often, and so are
...
\]
The product here is infinite (since, as we will see soon, there are infinitely
many primes), but only finitely many of its factors are $\neq1$.
\end{corollary}

The expression $n=\prod_{p\text{ prime}}p^{v_{p}\left(  n\right)  }$ is called
the \textbf{canonical factorization} of $n$.
...
\begin{proof}
WLOG assume that $m$ is nonzero (else, both statements are true).

WLOG assume that $n$ is nonzero (else, both statements are false).

WLOG assume that $m$ and $n$ are positive (since we can replace $n$ and $m$ by
$\left\vert n\right\vert $ and $\left\vert m\right\vert $, and use
$v_{p}\left(  a\right)  =v_{p}\left(  -a\right)  $).

$\Longleftarrow:$ Assume that each prime $p$ satisfies $v_{p}\left(  n\right)
\leq v_{p}\left(  m\right)  $.

But Corollary above yields%
\[
n=\prod_{p\text{ prime}}p^{v_{p}\left(  n\right)  }%
\ \ \ \ \ \ \ \ \ \ \text{and}\ \ \ \ \ \ \ \ \ \ m=\prod_{p\text{ prime}%
}p^{v_{p}\left(  m\right)  }.
\]
Thus $n\mid m$, since
\[
m=n\cdot\prod_{p\text{ prime}}\underbrace{p^{v_{p}\left(  m\right)
-v_{p}\left(  n\right)  }}_{\substack{\text{an integer,}\\\text{since }%
v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  }}.
\]


$\Longleftarrow:$ Assume $n\mid m$. We must prove that each prime $p$
satisfies $v_{p}\left(  n\right)  \leq v_{p}\left(  m\right)  $.

So let $p$ be a prime. From $n\mid m$, we obtain $m=nb$ for some integer $b$.
Thus, $v_{p}\left(  m\right)  =v_{p}\left(  nb\right)  =v_{p}\left(  n\right)
+\underbrace{v_{p}\left(  b\right)  }_{\geq0}\geq v_{p}\left(  n\right)  $.
\end{proof}

...
%TCIDATA{LastRevised=Saturday, February 16, 2019 13:26:50}
...
(For each $k\in\left\{  1,2,\ldots,1000\right\}  $, the ghost spends the
$k$-th night flipping switches $k,2k,3k,\ldots$.)

Which switches are on after $1000$ nights?

$1,4,9,\ldots$
\end{exercise}

\begin{proof}
[Solution.]In mathematical terms: Which positive integers $n$ have an odd
number of positive divisors? It's exactly the perfect squares, because...

Fix a positive integer $n$. Recall that the map%
\begin{align*}
\left\{  \text{positive divisors of }n\right\}   &  \rightarrow\left\{
\text{positive divisors of }n\right\}  ,\\
d  &  \mapsto n/d
...
\begin{theorem}
(Fermat's Little Theorem, short FLT)

Let $p$ be a prime. Let $a\in\mathbb{Z}$.

\textbf{(a)} If $p\nmid a$, then $a^{p-1}\equiv1\operatorname{mod}p$.

\textbf{(b)} Always, $a^{p}\equiv a\operatorname{mod}p$.
\end{theorem}

We won't prove this right now; we will derive it from Euler's theorem.

But let us observe why parts \textbf{(a)} and \textbf{(b)} of the FLT are equivalent.
...
Let $a$ and $n$ be coprime integers. Then, we can cancel $a$ from congruences
modulo $n$: That is, if $ab\equiv ac\operatorname{mod}n$ for two integers $b$
and $c$, then $b\equiv c\operatorname{mod}n$.
\end{lemma}

\begin{proof}
Let $ab\equiv ac\operatorname{mod}n$. Thus, $n\mid ab-ac=a\left(  b-c\right)
$. Since $n\perp a$, we can remove the $a$ here to get $n\mid b-c$.

(Alternative proof: $a$ has a modular inverse $a^{\prime}$ modulo $n$, since
$a\perp n$. Thus, multiplying the congruence $ab\equiv ac\operatorname{mod}n$
with $a^{\prime}$, and get $b\equiv c\operatorname{mod}n$.)
\end{proof}

Now, why are parts \textbf{(a)} of (\textbf{b)} of FLT equivalent?

[\textit{Part \textbf{(a)} }$\Longrightarrow$ \textit{part \textbf{(b)}:} If
$p\nmid a$, then $a^{p}\equiv a\operatorname{mod}p$ follows by multiplying
$a^{p-1}\equiv1\operatorname{mod}p$ by $a$. If $p\mid a$, then $a^{p}\equiv
a\operatorname{mod}p$ follows from the fact that both $a^{p}$ and $a$ are
$\equiv0\operatorname{mod}p$.

\textit{Part \textbf{(b)} }$\Longrightarrow$ \textit{part \textbf{(a)}:}
Assuming $p\nmid a$, we have $p\perp a$, and thus the Cancellation Lemma lets
us cancel $a$ from $a^{p}\equiv a\operatorname{mod}p$, so we obtain
$a^{p-1}\equiv1\operatorname{mod}p$.]

More general than part \textbf{(a)} is:
...
This theorem yields part \textbf{(a)} of the FLT, since any prime $p$
satisfies $\phi\left(  p\right)  =p-1$.

\begin{exercise}
What is the last digit of $3^{4^{5}}$ ?

(Note: $a^{b^{c}}$ means $a^{\left(  b^{c}\right)  }$, not $\left(
a^{b}\right)  ^{c}=a^{bc}$.)
\end{exercise}
...
Since $3$ is coprime to $10$, we can apply Euler's theorem and get
$3^{\phi\left(  10\right)  }\equiv1\operatorname{mod}10$. Since $\phi\left(
10\right)  =4$, this rewrites as $3^{4}\equiv1\operatorname{mod}10$. Now,%
\[
...
\begin{proof}
\textit{First proof.} WLOG $n>1$. Thus, neither $0$ nor $n$ is coprime to $n$.
So%
\begin{align*}
...
coprime to $n$ does not change. So%
\begin{align*}
&  \left\vert \left\{  i\in\left\{  0,1,\ldots,n-1\right\}  \ \mid\ i\perp
n\right\}  \right\vert \\
&  =\left\vert \left\{  i\in\left\{  1,2,\ldots,n\right\}  \ \mid\ i\perp
...
\]
Then, the lemma above yields $\left\vert C\right\vert =\phi\left(  n\right)  $.
...
thus $\left(  ai\right)  \%n\perp n$. Hence, $\left(  ai\right)  \%n\in C$, qed.]
...
(by one of the HW1 exercises). By the Cancellation Lemma, we can cancel $a$
from this congruence, and get $i\equiv j\operatorname{mod}n$. Since both $i$
and $j$ lie in $\left\{  0,1,\ldots,n-1\right\}  $, this entails $i=j$. So $f$
is injective.]
...
So $f$ is bijective. Thus, we can substitute $f\left(  c\right)  $ for $c$ in
the product $\prod\limits_{c\in C}c$. So%
\begin{align*}
...
\end{align*}
In other words, $z\equiv a^{\phi\left(  n\right)  }z\operatorname{mod}n$. The
Cancellation Lemma lets us cancel $z$ (since $z\perp n$), so this becomes
$1\equiv a^{\phi\left(  n\right)  }\operatorname{mod}n$. This proves Euler.
\end{proof}
...
Any fraction $\dfrac{a}{b}$ with $a,b\in\mathbb{Z}$ is periodic.

A fraction $\dfrac{a}{b}$ is called \textbf{purely periodic} if its period (in
...
(Reference: Keith Conrad, \textit{Euler's theorem}.)
\end{remark}
...
Recall the following properties of binomial coefficients:
...
\begin{theorem}
(Vandermonde convolution identity) Every $x,y\in\mathbb{Q}$ and $n\in
\mathbb{N}$ satisfy%
\[

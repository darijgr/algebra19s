...
%TCIDATA{LastRevised=Friday, February 01, 2019 12:02:30}
...
\begin{fineprint}
\begin{proof}
[Solution to Exercise \ref{exe.ent.div.acbc}.]We have%
\begin{align*}
& \ \left(  a\mid b\right)  \\
& \Longleftrightarrow\ \left(  \text{there exists an integer }k\text{ such
that }b=ak\right)  \\
& \Longleftrightarrow\ \left(  \text{there exists an integer }k\text{ such
that }bc=ack\right)  \\
& \ \ \ \ \ \ \ \ \ \ \left(
\begin{array}
[c]{c}%
\text{since }b=ak\text{ is equivalent to }bc=ack\\
\text{(since }c\neq0\text{)}%
\end{array}
\right)  \\
...
\begin{fineprint}
\begin{proof}
[Solution to Exercise \ref{exe.ent.mod.a+b=a-b}.]This is tantamount to saying
that $2\mid\left(  a+b\right)  -\left(  a-b\right)  $. But this follows from
$\left(  a+b\right)  -\left(  a-b\right)  =\allowbreak2b$.
\end{proof}
\end{fineprint}
...
\begin{fineprint}
\begin{proof}
\item[Solution to Exercise \ref{exe.ent.mod.basics.k-power}.] We want to prove
that
\begin{equation}
...
\begin{proof}
[Solution to Exercise \ref{exe.ent.mod.k-sum}.]???
\end{proof}
\end{fineprint}
...
\textbf{unique} pair $\left(  q,r\right)  \in\mathbb{Z}\times\left\{
0,1,\ldots,n-1\right\}  $ such that $u=qn+r$.
\end{theorem}

To prove this, we split the word \textquotedblleft unique\textquotedblright%
\ into two lemmas:

\begin{lemma}
(Existence) Let $n$ be a positive integer. Let $u\in\mathbb{Z}$. Then, there
exists \textbf{at least one} pair $\left(  q,r\right)  \in\mathbb{Z}%
...
\begin{proof}
[Proof of Uniqueness lemma.] Let $\left(  q_{1},r_{1}\right)  $ and $\left(
q_{2},r_{2}\right)  $ be two such pairs $\left(  q,r\right)  $. Our goal is to
prove $\left(  q_{1},r_{1}\right)  =\left(  q_{2},r_{2}\right)  $.

So $\left(  q_{1},r_{1}\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots
,n-1\right\}  $ satisfies $u=q_{1}n+r_{1}$.

So $\left(  q_{2},r_{2}\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots
,n-1\right\}  $ satisfies $u=q_{2}n+r_{2}$.

Now, $r_{1}$ and $r_{2}$ belong to the set $\left\{  0,1,\ldots,n-1\right\}
$. Hence, $\left\vert r_{2}-r_{1}\right\vert \leq n-1<n$.

On the other hand, comparing $u=q_{1}n+r_{1}$ with $u=q_{2}n+r_{2}$, we get
$q_{1}n+r_{1}=q_{2}n+r_{2}$. In other words,%
\begin{align*}
q_{1}n-q_{2}n  & =r_{2}-r_{1},\ \ \ \ \ \ \ \ \ \ \text{so}\\
r_{2}-r_{1}  & =q_{1}n-q_{2}n=\left(  q_{1}-q_{2}\right)
n,\ \ \ \ \ \ \ \ \ \ \text{so}\\
\left\vert r_{2}-r_{1}\right\vert  & =\left\vert \left(  q_{1}-q_{2}\right)
n\right\vert =\left\vert q_{1}-q_{2}\right\vert \cdot
n,\ \ \ \ \ \ \ \ \ \ \text{so}\\
\left\vert q_{1}-q_{2}\right\vert \cdot n  & =\left\vert r_{2}-r_{1}%
\right\vert <n.
\end{align*}
We can divide this inequality by $n$ (since $n>0$) and get $\left\vert
q_{1}-q_{2}\right\vert <1$. Since $\left\vert q_{1}-q_{2}\right\vert $ is a
nonnegative integer, this means $\left\vert q_{1}-q_{2}\right\vert =0$. So
$q_{1}=q_{2}$. Recalling $r_{2}-r_{1}=\left(  q_{1}-q_{2}\right)  n$, we
easily derive $r_{1}=r_{2}$. So $\left(  q_{1},r_{1}\right)  =\left(
q_{2},r_{2}\right)  $. The Uniqueness Lemma is thus proven.
\end{proof}

We split the proof of the Existence Lemma into two steps. The first is proving
it for nonnegative $u$:

\begin{lemma}
(Existence in nonnegative case) Let $n$ be a positive integer. Let
...
\begin{proof}
Strong induction on $u$:

Fix $U\in\mathbb{N}$.

Assume (as the induction hypothesis) that the lemma is true for all $u<U$.

We must then prove that the lemma is true for $u=U$. In other words, we must
prove that there exists \textbf{at least one} pair $\left(  q,r\right)
...
\textit{Case 2:} $U\geq n$.

Let us first consider Case 1. Here, $U<n$. Thus, $U\in\left\{  0,1,\ldots
,n-1\right\}  $. So the pair $\left(  0,U\right)  $ is already a pair $\left(
q,r\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots,n-1\right\}  $ such that
$U=qn+r$. Thus, the lemma holds for $u=U$ in Case 1.

Let us now consider Case 2. Here, $U\geq n$. Thus, $U-n\in\mathbb{N}$ and
$U-n<U$. Hence, the induction hypothesis says that the lemma is true for
$u=U-n$. In other words, there exists \textbf{at least one} pair $\left(
q,r\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots,n-1\right\}  $ such that
$U-n=qn+r$. Fix such a pair and call it $\left(  q_{0},r_{0}\right)  $.

So $\left(  q_{0},r_{0}\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots
,n-1\right\}  $ and $U-n=q_{0}n+r_{0}$. The latter equality yields%
\[
U=n+q_{0}n+r_{0}=\left(  q_{0}+1\right)  n+r_{0}.
\]
Also, $\left(  q_{0}+1,r_{0}\right)  \in\mathbb{Z}\times\left\{
0,1,\ldots,n-1\right\}  $. Thus, there exists a pair $\left(  q,r\right)
\in\mathbb{Z}\times\left\{  0,1,\ldots,n-1\right\}  $ such that $U=qn+r$
(namely, $\left(  q_{0}+1,r_{0}\right)  $). So the lemma holds for $u=U$ in
Case 2.

So the lemma always holds for $u=U$. So the induction is finished. So the
lemma is proven.
...
\begin{proof}
We have already proven this in the case when $u\in\mathbb{N}$. So let us now
assume that $u\notin\mathbb{N}$. Thus, $u<0$. So $-u\in\mathbb{N}$.

Set $v=\left(  n-1\right)  \left(  -u\right)  $. Then, $v\in\mathbb{N}$ (since
$n-1\in\mathbb{N}$ and $-u\in\mathbb{N}$). Thus, the previous lemma (applied
to $v$ instead of $u$) shows that there exists \textbf{at least one} pair
$\left(  q,r\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots,n-1\right\}  $
such that $v=qn+r$. Fix such a pair and call it $\left(  q_{0},r_{0}\right)  $.

Thus, $\left(  q_{0},r_{0}\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots
,n-1\right\}  $ and $v=q_{0}n+r_{0}$.

Now, check that $\left(  q_{0}+u,r_{0}\right)  \in\mathbb{Z}\times\left\{
0,1,\ldots,n-1\right\}  $ and $u=\left(  q_{0}+u\right)  n+r_{0}$. (Let's
check:%
\[
q_{0}n+r_{0}=v=\left(  n-1\right)  \left(  -u\right)  =-nu+u,
\]
so $u=q_{0}n+r_{0}+nu=\left(  q_{0}+u\right)  n+r_{0}$.)

Thus, there exists \textbf{at least one} pair $\left(  q,r\right)
\in\mathbb{Z}\times\left\{  0,1,\ldots,n-1\right\}  $ such that $u=qn+r$
(namely, $\left(  q_{0}+u,r_{0}\right)  $). So the lemma is proven.
\end{proof}

\begin{definition}
Let $n$ be a positive integer. Let $u\in\mathbb{Z}$. The Theorem says that
there exists a \textbf{unique} pair $\left(  q,r\right)  \in\mathbb{Z}%
\times\left\{  0,1,\ldots,n-1\right\}  $ such that $u=qn+r$.

We call $q$ the \textit{quotient} of the division of $u$ by $n$, and denote it
by $u//n$.

We call $r$ the \textit{remainder} of the division of $u$ by $n$, and denote
it by $u\%n$.
\end{definition}
...
\textbf{(c)} If $c\in\left\{  0,1,\ldots,n-1\right\}  $ satisfies $c\equiv
...
\begin{proof}
We defined $u//n$ and $u\%n$ in such a way that $\left(  u//n,u\%n\right)  $
is the \textbf{unique} pair $\left(  q,r\right)  \in\mathbb{Z}\times\left\{
0,1,\ldots,n-1\right\}  $ such that $u=qn+r$.

Thus, $\left(  u//n,u\%n\right)  \in\mathbb{Z}\times\left\{  0,1,\ldots
,n-1\right\}  $ and $u=\left(  u//n\right)  n+\left(  u\%n\right)  $. This
proves part \textbf{(d)}.

Also, $u\%n\in\left\{  0,1,\ldots,n-1\right\}  $. To prove $u\%n\equiv
u\operatorname{mod}n$, we observe that%
\[
\left(  u\%n\right)  -u=-\left(  u//n\right)  n\text{ is divisible by }n.
\]
Thus, part \textbf{(a)} is proven.

\textbf{(c)} Let $c\in\left\{  0,1,\ldots,n-1\right\}  $ satisfy $c\equiv
u\operatorname{mod}n$. So $n\mid c-u$, so there exists some $k\in\mathbb{Z}$
such that $c-u=kn$. Thus, $u=c-kn=\left(  -k\right)  n+c$. Thus, $\left(
-k,c\right)  $ is a pair $\left(  q,r\right)  \in\mathbb{Z}\times\left\{
0,1,\ldots,n-1\right\}  $ such that $u=qn+r$. But we know that $\left(
u//n,u\%n\right)  $ is the \textbf{unique} such pair. So $\left(  -k,c\right)
=\left(  u//n,u\%n\right)  $. So $-k=u//n$ and $c=u\%n$. This proves
\textbf{(c)}.

\textbf{(b)} $\Longrightarrow:$ Assume $n\mid u$. Thus, there exists
$k\in\mathbb{Z}$ such that $u=nk$. Consider this $k$. Now, $u=nk=kn+0$. Hence,
$\left(  k,0\right)  $ is a pair $\left(  q,r\right)  \in\mathbb{Z}%
\times\left\{  0,1,\ldots,n-1\right\}  $ such that $u=qn+r$. But we know that
$\left(  u//n,u\%n\right)  $ is the \textbf{unique} such pair. So $\left(
k,0\right)  =\left(  u//n,u\%n\right)  $. So $k=u//n$ and $0=u\%n$. Thus
$u\%n=0$.

$\Longleftarrow:$ Assume $u\%n=0$. Then, $u=\left(  u//n\right)
n+\underbrace{\left(  u\%n\right)  }_{=0}=\left(  u//n\right)  n$, so $n\mid
u$.
\end{proof}

\begin{exercise}
Let $n$ be a positive integer. Let $u,v\in\mathbb{Z}$. Prove that $u\equiv

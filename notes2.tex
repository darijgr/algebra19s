...
%TCIDATA{LastRevised=Monday, February 18, 2019 12:04:54}
...
\textit{Base case:} $n=0$. Then, $\dbinom{n}{k}=\dbinom{0}{k}=%
\begin{cases}
1, & \text{if }k=0;\\
0, & \text{if }k\neq0
\end{cases}
$ (follows from the definition). On the other hand, $N=\varnothing$ (since
$\left\vert N\right\vert =n=0$). Thus, its only subset is $\varnothing$, which
is a $0$-element subset. Hence, the number of $k$-element subsets of $N$ is $%
\begin{cases}
1, & \text{if }k=0;\\
0, & \text{if }k\neq0
\end{cases}
...
\textit{Step:} Let $n>0$. Assume that the theorem holds for $n-1$ instead of
$n$.
...
The number of Type-1 subsets is $\dbinom{n-1}{k}$ (since the Type-1 subsets
are precisely the $k$-element subsets of the $\left(  n-1\right)  $-element
set $N\setminus\left\{  a\right\}  $).
...
& =\dbinom{n-1}{k-1}\ \ \ \ \ \ \ \ \ \ \left(  \text{by induction
hypothesis}\right)  .
\end{align*}
)
...
\begin{proof}
If $n\geq0$, then this follows from the Corollary. Thus, WLOG assume that
$n<0$.

WLOG assume that $k\in\mathbb{N}$ (otherwise, $\dbinom{n}{k}=0$).
...
$p$ is prime). But since $p\nmid k$, we thus have $p\mid\dbinom{p}{k}$.

[Second proof (outline).] Consider \textit{necklaces} with $p$ beads colored
either white or black such that exactly $k$ beads are black. For now, let us
\textbf{not} equate necklaces that differ only by rotation; so a necklace is
just a $p$-tuple of colors such that $k$ of its entries are \textquotedblleft
black\textquotedblright\ and the remaining ones are \textquotedblleft
white\textquotedblright.

Thus, there are $\dbinom{p}{k}$ necklaces. But each such necklace gives rise
to $p-1$ further necklaces by rotating it cyclically. So the necklaces are
grouped into bunches of $p$ necklaces each. And thus their number is divisible
by $p$. (We will later make this rigorous, when we learn about group actions.)
\end{proof}
...
\begin{proof}
[Proof outline.] \textit{Step 1:} Let us prove the Theorem in the case when
$x,y\in\mathbb{N}$.

So let $x,y\in\mathbb{N}$. Let%
\[
...
\begin{itemize}
\item The number of $n$-element subsets of $A$ is $\dbinom{x+y}{n}$, since $A$
is an $\left(  x+y\right)  $-element set.

\item Let us classify such subsets according to how many positive elements
they have. For each $k\in\left\{  0,1,\ldots,n\right\}  $, the number of
$n$-element subsets of $A$ having \textbf{exactly }$k$ \textbf{positive
elements} is $\dbinom{x}{k}\dbinom{y}{n-k}$. Thus, the total number of
$n$-element subsets of $A$ is $\sum\limits_{k=0}^{n}\dbinom{x}{k}\dbinom
{y}{n-k}$.
\end{itemize}
...
\textit{Step 2:} We shall prove the Theorem for $x\in\mathbb{Q}$ and
$y\in\mathbb{N}$.

To do so, we fix $y\in\mathbb{N}$, but let $x$ stay variable.

Note that $\dbinom{x+y}{n}$ is a polynomial in $x$, since%
\[
...
$x$. Now, recall (from April?) the following fact:

\begin{statement}
\textit{Fact:} If $P$ and $Q$ are two polynomials in a variable $x$, and if
infinitely many $x\in\mathbb{Q}$ satisfy $P\left(  x\right)  =Q\left(
x\right)  $, then $P=Q$ as polynomials.
\end{statement}

[We will prove this in detail in April or so.]

Apply the Fact to $P=\dbinom{x+y}{n}$ and $Q=\sum\limits_{k=0}^{n}\dbinom
{x}{k}\dbinom{y}{n-k}$. Step 1 tells us that $P\left(  x\right)  =Q\left(
x\right)  $ for all $x\in\mathbb{N}$. Thus, infinitely many $x\in\mathbb{Q}$
satisfy $P\left(  x\right)  =Q\left(  x\right)  $. Hence, $P=Q$ (by Fact).
This means that $\dbinom{x+y}{n}=\sum\limits_{k=0}^{n}\dbinom{x}{k}\dbinom
{y}{n-k}$ holds for \textbf{all} $x\in\mathbb{Q}$.
...
$a$ and $b$ in the $p$-ary system as%
\[
